...
/***************************************
  ...
  功能描述:	外部证书上传(620)
 ***************************************/
int UnionDealServiceCode8ED3(PUnionHsmGroupRec phsmGroupRec)
{
	int			ret;
	char			tmpBuf[128];
	char			appName[128];
	...
	// 文件名
        ...
        {
                UnionUserErrLog("in UnionDealServiceCode8ED3:: UnionReadRequestXMLPackageValue[%s]!\n","body/fileName");
                return(ret);
        }
        UnionFilterHeadAndTailBlank(fileName);
        if (strlen(fileName) == 0)
        {
                UnionUserErrLog("in UnionDealServiceCode8ED3:: fileName can not be null!\n");
	...
	// 读取文件类型
        ...
        {
                UnionUserErrLog("in UnionDealServiceCode8ED3:: fileTypeID can not be null!\n");
        ...
        {
                UnionUserErrLog("in UnionDealServiceCode8ED3:: UnionSelectRealDBRecord[%s]!\n",sql);
                return(ret);
        }
        else if (ret == 0)
        {
                UnionUserErrLog("in UnionDealServiceCode8ED3:: fileTypeID[%s]!\n",fileTypeID);
                return(errCodeDatabaseMDL_RecordNotFound);
        }
	
        if ((ret = UnionLocateXMLPackage("detail",1)) < 0)
        {
                UnionUserErrLog("in UnionDealServiceCode8ED3:: UnionLocateXMLPackage[%s]!\n","detail");
                return(ret);
        }
        ...
        {
                UnionUserErrLog("in UnionDealServiceCode8ED3:: UnionReadXMLPackageValue[%s]!\n","fileStoreDir");
                return(ret);
        }
	...
	// 检查文件是否存在
        ...
        {
                UnionUserErrLog("in UnionDealServiceCode8ED3:: fullFileName[%s] not found!\n",fullFileName);
	...
	{
                UnionUserErrLog("in UnionDealServiceCode8ED3:: fopen[%s]!\n",fullFileName);
                return(errCodeUseOSErrCode);
	}
	if ((certLen = fread(certDer, 1, sizeof(certDer) - 1, fp)) < 0)
	{
                UnionUserErrLog("in UnionDealServiceCode8ED3:: fread[%s]!\n",fullFileName);
                return(errCodeUseOSErrCode);
	}
	fclose(fp);
	certDer[certLen] = 0;
	...
	{
		UnionUserErrLog("in UnionDealServiceCode8ED3:: UnionGetHsmGroupRecByHsmGroupID!\n");
		return(ret);
	}
	if (pSpecHsmGroup.hsmCmdVersionID != conHsmCmdVerSRJ1401StandardHsmCmd)
	{
		UnionUserErrLog("in UnionDealServiceCode8ED3:: phsmGroupRec->hsmCmdVersionID[%d]\n",phsmGroupRec->hsmCmdVersionID);
	...
	case conHsmCmdVerSRJ1401StandardHsmCmd:
		if ((ret = UnionSRJ1401CmdES620(appName,bankName,(unsigned char*)cert,certLen, &upload)) < 0)
		{
			UnionUserErrLog("in UnionDealServiceCode8ED3:: UnionSRJ1401CmdES620,ret = [%d]!\n",ret);
			return(ret);
		}
		break;
	default:
