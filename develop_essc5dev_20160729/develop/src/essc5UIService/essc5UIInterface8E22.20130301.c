...
/***************************************
  ...
  功能描述:       删除证书
 ***************************************/
int UnionDealServiceCode8E22(PUnionHsmGroupRec phsmGroupRec)
{
	int                             ret;
	char                            sql[128];
	//int                             vkIdx = 0;
	...
	{
		UnionUserErrLog("in UnionDealServiceCode8E22:: UnionReadRequestXMLPackageValue[%s]!\n","body/keyName");
		return(ret);
	}
	UnionFilterHeadAndTailBlank(asymmetricKeyDB.keyName);
	if (strlen(asymmetricKeyDB.keyName) == 0)
	{
		UnionUserErrLog("in UnionDealServiceCode8E22:: keyName can not be null!\n");
	...
	{
		UnionUserErrLog("in UnionDealServiceCode8E22:: UnionReadAsymmetricKeyDBRec[%s]!\n",asymmetricKeyDB.keyName);
		return(ret);
	}
	...
	if (asymmetricKeyDB.vkStoreLocation != 0)
	{
		...
		{
			UnionUserErrLog("in UnionDealServiceCode8E22:: UnionExecRealDBSql ret = [%d] sql = [%s]!\n",ret,sql);
			return(ret);
		}
	}
	...
	{
		UnionUserErrLog("in UnionDealServiceCode8E22:: UnionDropAsymmetricKeyDB keyName[%s]!\n",asymmetricKeyDB.keyName);
		return(ret);
	}

	memset(sql,0,sizeof(sql));
	sprintf(sql,"delete from certOfCnaps2 where keyName= '%s'",asymmetricKeyDB.keyName);
	if ((ret = UnionExecRealDBSql(sql)) < 0)
	{
		UnionUserErrLog("in UnionDealServiceCode8E22:: UnionExecRealDBSql ret = [%d] sql = [%s]!\n",ret,sql);
		return(ret);
