...
int UnionDealServiceCodeE405(PUnionHsmGroupRec phsmGroupRec)
{
	int				ret=0;
	char 				filePath[512];
	...
	if ((ret = UnionReadRequestXMLPackageValue("body/pix",pix,sizeof(pix))) < 0)
        {
                UnionUserErrLog("in UnionDealServiceCodeE405:: UnionReadRequestXMLPackageValue[%s]!\n","body/pix");
                return(ret);
        }
	else
        {
		...
		if (strlen(pix) == 0)
                {
                        UnionUserErrLog("in UnionDealServiceCodeE405:: pix can not be null!\n");
                        UnionSetResponseRemark("应用ID不能为空!");
                        return(errCodeParameter);
                }
                if (!UnionIsBCDStr(pix))
                {
                        UnionUserErrLog("in UnionDealServiceCodeE405:: pix[%s] is error!\n",pix);
	...
	if (strlen(bankID) == 0)
	{
		UnionUserErrLog("in UnionDealServiceCodeE405:: bankID can not be null!\n");
	...
	{
		UnionUserErrLog("in UnionDealServiceCodeE405:: mode[%s] error!!\n",mode);
	...
	if (mode[0] != 2)
	{
		...
		if (lenOfSign == 0)
		{
			UnionUserErrLog("in UnionDealServiceCodeE405:: sign can not be null!\n");
			return(errCodeParameter);
		}
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE405:: UnionParaseJHSignWithPkcs7 [%s] [%d]!\n",pkcs7Sign,ret);
			return(ret);
		}
	}
	...
	if(!algID)
	{
		...
		if (mode[0] != '0')
		{
			...
			{
				UnionUserErrLog("in UnionDealServiceCodeE405:: UnionReadAsymmetricKeyDBRec asymmetricKeyDB.keyName[%s]!\n",asymmetricKeyDB.keyName);
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE405:: UnionGetCertificateInfoFromBuf[%s] [%d]!\n",cerBuf,ret);
			return(ret);
		}
		if ((strncmp(sysDate, x509CerSM2.startDateTime, 8) < 0) || (strncmp(sysDate, x509CerSM2.endDataTime, 8) > 0))
		{
			UnionUserErrLog("in UnionDealServiceCodeE405:: certificate out of date!\n");
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE405:: create file [%s] failed!\n",x509FileName);
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE405:: UnionPKCSCerASN1DER_Decode[%s] [%d]!\n",x509CerSM2.derPK,ret);
			return(ret);
		}
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE405:: evalue[%s] != 010001 or 3!\n",evalue);
			return(ret);
		}
		...
		*/
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE405:: UnionTranslateStringTimeToTime dateTime[%s] length != 14!\n",dateTime);
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE405:: UnionVerifyCertByP7bCAlinks  x509FileName[%s]  p7bFullName[%s]!\n",x509FileName,p7bFileName);
			return(ret);
		}
		*/
		...
		if (mode[0] == '0')	// 增加密钥
		{
			...
			{
				UnionUserErrLog("in UnionDealServiceCodeE405:: UnionInitAsymmetricKeyDB!\n");
				return(ret);
			}
			if ((ret = UnionGenerateAsymmetricKeyDBRec(&asymmetricKeyDB)) < 0)
			{
				UnionUserErrLog("in UnionDealServiceCodeE405:: UnionGenerateAsymmetricKeyDBRec[%s]!\n",asymmetricKeyDB.keyName);
				return(ret);
			}
			/*
			// 发布证书者的用户信息
			...
			*/
			...
			{
				UnionUserErrLog("in UnionDealServiceCodeE405:: UnionExecRealDBSql[%s]!\n",sql);
				return(ret);
			}
		}
		else if (mode[0] == '1')	// 更新密钥
		{
			...
			{
				UnionUserErrLog("in UnionDealServiceCodeE405:: UnionUpdateAsymmetricKeyDBKeyValue[%s]!\n",asymmetricKeyDB.keyName);
				return(ret);
			}
			// 更新证书信息
	...
	else
	{
		...
		if ((ret = UnionGetCertificateInfoFromBufEx(cerBuf, cerBufLen, &x509CerSM2)) < 0)
		{
			UnionUserErrLog("in UnionDealServiceCodeE405:: UnionGetCertificateInfoFromBufEx[%s] [%d]!\n", cerBuf, ret);
			return(ret);
		}
		/*
		// 发布证书者的用户信息
		...
		*/
		...
		if((ret = UnionSelectRealDBRecord(sql, 0, 0)) < 0)
		{
			UnionUserErrLog("in UnionDealServiceCodeE405:: UnionSelectRealDBRecord[%s]!\n", sql);
			return(ret);
		}
		else if(ret == 0)
		{
			UnionUserErrLog("in UnionDealServiceCodeE405:: rootCert[%s] record not found!\n", x509CerSM2.issuserDN);
			return(errCodeOffsetOfCertMDL_CaNotExist);
		}
		UnionLocateXMLPackage("detail", 1);
		if((ret = UnionReadXMLPackageValue("keyName", caAsymmetricKeyDB.keyName, sizeof(caAsymmetricKeyDB.keyName))) < 0)
