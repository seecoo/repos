...
int UnionDealServiceCodeE151(PUnionHsmGroupRec phsmGroupRec)
{
	...
	int				algGrpLen = 8; // des 8B分组 sm4 16B
	int				msgNum = 0;
	char				tmpBuf[32];
	char				data[8192];
	char				ascData[8192*2];
	...
	if (mode == 1 || mode == 5)//modify by huangh 20160525,增加了mode==5模式，该模式为湖南农信专用
	{
		// 密钥名称
	...
	if ((lenOfMac = UnionReadRequestXMLPackageValue("body/mac",mac,sizeof(mac))) < 0)
	{
		...
		lenOfMac = strlen(mac);
                if (lenOfMac < 8)
                {
                        ...
                        UnionSetResponseRemark("mac不能为空或长度小于8!");
                        return(errCodeParameter);
                }
		if (lenOfMac >16)
		{
			lenOfMac = 16;
                ...
                if (!UnionIsBCDStr(mac))
                {
                        ...
                        UnionSetResponseRemark("mac[%s]非法,必须为十六进制数",mac);
                        return(errCodeParameter);
                }
        }
