...
int UnionDealServiceCodeE100(PUnionHsmGroupRec phsmGroupRec)
{
	...
	if ((ret = UnionReadRequestXMLPackageValue("body/hsmIP",hsmIP,sizeof(hsmIP))) <= 0)
	{
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE100:: UnionReadRequestXMLPackageValue[%s]!\n","head/appID");
			return(ret);
		}
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE100:: UnionSelectRealDBRecord[%s]!\n",sql);
			return(ret);
		}
		else if(ret == 0)
		{
			UnionUserErrLog("in UnionDealServiceCodeE100:: appID[%s]!\n",appID);
			return(errCodeEssc_AppNotDefined);
		}
		if ((ret = UnionLocateXMLPackage("detail", 1)) < 0)
		{
			UnionUserErrLog("in UnionDealServiceCodeE100:: UnionLocateXMLPackage[%s]!\n","detail");
			return(ret);
		}
		if ((ret = UnionReadXMLPackageValue("hsmGroupIDList",hsmGroupIDList,sizeof(hsmGroupIDList))) < 0)
		{
			UnionUserErrLog("in UnionDealServiceCodeE100:: UnionReadXMLPackageValue[%s]!\n","hsmGroupIDList");
			return(ret);
		}
	...
	else
	{
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE100:: hsmIP[%s] error!\n",hsmIP);
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE100:: hsmIP[%s]!\n",hsmIP);
	...
	if (ipSpecHsm)
	{
		...
		if (status[0] != '1')
		{
			UnionUserErrLog("in UnionDealServiceCodeE100:: hsmIP[%s] not online!\n",hsmIP);
	...
	switch(phsmGroupRec->hsmCmdVersionID)
	{
		...
		case conHsmCmdVerSJL06StandardHsmCmd:
			...
			{
				UnionUserErrLog("in UnionDealServiceCodeE100:: UnionDirectHsmCmd,ret = [%d]!\n",ret);
				return(ret);
			}
			break;
		default:
			UnionUserErrLog("in UnionDealServiceCodeE100:: phsmGroupRec->hsmCmdVersionID[%d]\n",phsmGroupRec->hsmCmdVersionID);
	...
	{
		UnionUserErrLog("in UnionDealServiceCodeE100:: hsmCmdRes[%s] != resCmdOfTest[%s]!\n",hsmCmdRes,resCmdOfTest);
