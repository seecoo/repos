...
int UnionDealServiceCodeE110(PUnionHsmGroupRec phsmGroupRec)
{
	...
	*/
	...
	else
	{
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE110:: mode[%s] error!\n",tmpBuf);
			return(errCodeParameter);
		}
	}
	...
	if (strlen(symmetricKeyDB.keyName) > 0)
	{
		if ((ret =  UnionReadSymmetricKeyDBRec(keyName,0,&symmetricKeyDB)) != errCodeKeyCacheMDL_WrongKeyName)
		{
			if (mode == 2)
			{
				if ((ret = UnionDealServiceCodeE116(phsmGroupRec)) < 0)
				{
					UnionUserErrLog("in UnionDealServiceCodeE110:: UnionDealServiceCodeE116!\n");
					return(ret);
				}
				memset(&symmetricKeyDB,0,sizeof(symmetricKeyDB));
				snprintf(symmetricKeyDB.keyName,sizeof(symmetricKeyDB.keyName),"%s",keyName);
			...
			{
				if (updateKeyFlag[0] == '1')	// 密钥存在,且更新标识为1,则调用E111服务更新密钥
				{
					if ((ret = UnionDealServiceCodeE111(phsmGroupRec)) < 0)
					{
						UnionUserErrLog("in UnionDealServiceCodeE110:: UnionDealServiceCodeE111!\n");
						return(ret);
					}
					return 0;
				}
				...
				{	
					UnionUserErrLog("in UnionDealServiceCodeE110:: 对称密钥[%s]已存在!\n",keyName);
					UnionSetResponseRemark("对称密钥[%s]已存在",keyName);
	...
	{
		UnionUserErrLog("in UnionDealServiceCodeE110:: UnionReadRequestXMLPackageValue[%s]!\n","body/keyType");
		return(ret);
	}
	tmpBuf[ret] = 0;
	...
	else	
	{
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE110:: keyType[%s] error!\n",tmpBuf);
	...
	{
		UnionUserErrLog("in UnionDealServiceCodeE110:: algorithmID[%s] error!\n",tmpBuf);
	...
	if (!UnionIsDigitString(tmpBuf))
	{
		UnionUserErrLog("in UnionDealServiceCodeE110:: keyLen[%s] is invalid!\n",tmpBuf);
	...
	{
		UnionUserErrLog("in UnionDealServiceCodeE110:: oldVersionKeyIsUsed[%s] error!\n",tmpBuf);
		return(errCodeParameter);
	}
	...
	else
	{
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE110:: inputFlag[%s] error!\n",tmpBuf);
			return(errCodeParameter);
		}
	}
	...
	else
	{
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE110:: outputFlag[%s] error!\n",tmpBuf);
			return(errCodeParameter);
		}
	}
	...
	// 1：指定密钥名称保护输出
	// 2：外带ZMK密钥保护输出
	// 3：外带TMK密钥保护输出(keyType必须是：TMK、TPK、TAK)
	if (symmetricKeyDB.outputFlag == 1)
	{
	...
	else
	{
		...
		if (!UnionIsDigitString(tmpBuf))
		{
			UnionUserErrLog("in UnionDealServiceCodeE110:: effectiveDays[%s] error!\n",tmpBuf);
	...
	UnionLog("in UnionDealServiceCodeE110:: mode[%d] inputFlag[%d] outputFlag[%d] enabled[%d] exportFlag[%d],isRemoteKeyOperate[%d]\n",mode,inputFlag,outputFlag,enabled,exportFlag,isRemoteKeyOperate);
	...
	else if (exportFlag == 1)
	{
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE110:: UnionReadSymmetricKeyDBRec protectKey[%s]!\n",protectKey);
			return(ret);
		}
		if ((zmkKeyDB.keyType != conZMK) && (zmkKeyDB.keyType != conTMK))
		{
			UnionUserErrLog("in UnionDealServiceCodeE110:: is not zmk or tmk [%s]!\n",protectKey);
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE110:: UnionGetSymmetricKeyValue lmkProtectMode[%s]!\n",phsmGroupRec->lmkProtectMode);
		...
		if (zmkKeyDB.keyType == conTMK)
		{
			...
			{
				UnionUserErrLog("in UnionDealServiceCodeE110:: keyName[%s] keyType is not tmk,tpk,pvk or tak!\n",symmetricKeyDB.keyName);
	...
	else
	{
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE110:: protectKey[%s] len != [16,32,48]\n",protectKey);
			return(errCodeEssc_KeyLength);
		}
		if (exportFlag == 3)
		{
	...
	if (!isRemoteKeyOperate)		// 本地密钥操作	
	{
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE110:: UnionCheckRemoteKeyPlatform!\n");
			return(ret);
		}
		// modify by leipp end
	}
	else			// 远程密钥操作
	{
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE110:: UnionSelectRealDBRecord[%s]\n",sql);
			return(ret);
		}
		else if (ret == 0)
		{
			UnionUserErrLog("in UnionDealServiceCodeE110:: protectKey not found!\n");
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE110:: UnionLocateXMLPackage[%s]!\n","detail");	
			return(ret);
		}

		if ((ret = UnionReadXMLPackageValue("protectKey",platformProtectKey,sizeof(platformProtectKey))) < 0)
		{
			UnionUserErrLog("in UnionDealServiceCodeE110:: UnionReadXMLPackageValue[%s]!\n","protectKey");
			return(ret);
		}
	...
	if (isRemoteApplyKey)		// 远程申请密钥
	{
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE110:: UnionInitRemoteRequestPackageToRemoteKeyPlatform!\n");
			return(ret);
		}
		// 转发密钥操作到远程平台
		if ((ret = UnionTransferKeyOperateToRemoteKeyPlatform(&tkeyApplyPlatform)) < 0)
		{
			UnionUserErrLog("in UnionDealServiceCodeE110:: UnionTransferKeyOperateToRemoteKeyPlatform!\n");
			return(ret);
		}
	}
	...
	if (mode == 1)	// 生成密钥
	{
		if (isRemoteApplyKey)	// 远程申请
		{
			...
			{
				UnionUserErrLog("in UnionDealServiceCodeE110:: UnionReadResponseRemoteXMLPackageValue[%s]!\n","body/keyValue");
				return(ret);
			}
				...
				case	conSymmetricAlgorithmIDOfDES:	// DES算法
					...
					switch(phsmGroupRec->hsmCmdVersionID)
					{
						...
						case	conHsmCmdVerSJL06StandardHsmCmd:
							...
							{
								UnionUserErrLog("in UnionDealServiceCodeE110:: UnionHsmCmdA6!\n");
								return(ret);
							}
							break;
						default:
							UnionUserErrLog("in UnionDealServiceCodeE110:: phsmGroupRec->hsmCmdVersionID[%d]\n",phsmGroupRec->hsmCmdVersionID);
					...
					{
						UnionUserErrLog("in UnionDealServiceCodeE110:: symmetricKeyDB.checkValue[%s] != remoteCheckValue[%s]\n",symmetricKeyDB.checkValue,remoteCheckValue);
							...
							case	conHsmCmdVerSJL06StandardHsmCmd:
								if (((exportFlag == 1) && (zmkKeyDB.keyType == conTMK)) || (exportFlag == 3))
								{
									...
									if (symmetricKeyDB.keyType == conTMK || symmetricKeyDB.keyType == conTPK || symmetricKeyDB.keyType == conPVK)
									{
										...
										{
											UnionUserErrLog("in UnionDealServiceCodeE110:: UnionHsmCmdAE!\n");
											return(ret);
										}
									...
									else if (symmetricKeyDB.keyType == conTAK)
									{
										...
										{
											UnionUserErrLog("in UnionDealServiceCodeE110:: UnionHsmCmdAG!\n");
											return(ret);
										}
										strcpy(keyByZMK,keyByTMK);
									}
								...
								else	// zmk保护输出
								{
									...
									{
										UnionUserErrLog("in UnionDealServiceCodeE110:: UnionHsmCmdA8!\n");
										return(ret);
									}
				...
				case	conSymmetricAlgorithmIDOfSM4:	// SM4算法
					...
					switch(phsmGroupRec->hsmCmdVersionID)
					{
						...
						case conHsmCmdVerSJL06StandardHsmCmd:
							...
							{
								UnionUserErrLog("in UnionDealServiceCodeE110:: UnionHsmCmdSV!\n");
								return(ret);
							}
							break;
						default:
					...
					if (outputByZMK == 1)
					{
						...
						switch(phsmGroupRec->hsmCmdVersionID)
						{
							...
							case conHsmCmdVerSJL06StandardHsmCmd:
								...
								{
									UnionUserErrLog("in UnionDealServiceCodeE110:: UnionHsmCmdST!\n");
									return(ret);
								}
								break;
							default:
								UnionUserErrLog("in UnionDealServiceCodeE110:: phsmGroupRec->hsmCmdVersionID[%d] error\n",phsmGroupRec->hsmCmdVersionID);
								UnionSetResponseRemark("非法的加密机指令类型");
								return(errCodeParameter);
						}
						// 检查校验值
					...
					break;
				default:
					UnionUserErrLog("in UnionDealServiceCodeE110:: symmetricKeyDB.algorithmID[%d] error!\n",symmetricKeyDB.algorithmID);
					return(errCodeEsscMDL_InvalidAlgorithmID);
			}			
		}
		else if (isDistributeFlag)	// 远程分发
		{
			...
			switch(symmetricKeyDB.algorithmID)
			{
				...
				case	conSymmetricAlgorithmIDOfSM4:	// SM4算法
					...
					break;
				default:
					UnionUserErrLog("in UnionDealServiceCodeE110:: Distribute Key symmetricKeyDB.algorithmID[%d] error!\n",symmetricKeyDB.algorithmID);
					return(errCodeEsscMDL_InvalidAlgorithmID);
			}			
				...
				case	conSymmetricAlgorithmIDOfDES:	// DES算法
					switch(phsmGroupRec->hsmCmdVersionID)
					{
						...
						case conHsmCmdVerSJL06StandardHsmCmd:
							...
							{
								UnionUserErrLog("in UnionDealServiceCodeE110:: UnionHsmCmdA0!\n");
								return(ret);
							}
					...
					if (isApplyFlag) // 远程
					{
						switch(phsmGroupRec->hsmCmdVersionID)
						{
							...
							case conHsmCmdVerSJL06StandardHsmCmd:
								...
								{
									UnionUserErrLog("in UnionDealServiceCodeE110:: Distribute Key UnionHsmCmdA8!\n");
									return(ret);
								}
								break;
							default:
						...
						{
							UnionUserErrLog("in UnionDealServiceCodeE110:: Distribute Key symmetricKeyDB.checkValue[%s] != remoteCheckValue[%s]\n",symmetricKeyDB.checkValue,remoteCheckValue);
							return(errCodeEssc_CheckValue);
						}					
					}
					break;
				case	conSymmetricAlgorithmIDOfSM4:	// SM4算法
					...
					switch(phsmGroupRec->hsmCmdVersionID)
					{
						...
						case conHsmCmdVerSJL06StandardHsmCmd:
							...
							{
								UnionUserErrLog("in UnionDealServiceCodeE110:: UnionHsmCmdWI!\n");
								return(ret);
							}
							break;
						default:
					...
					if (isApplyFlag) // 远程
					{
						switch(phsmGroupRec->hsmCmdVersionID)
						{
							...
							case conHsmCmdVerSJL06StandardHsmCmd:
								...
								{
									UnionUserErrLog("in UnionDealServiceCodeE110:: Distribute Key UnionHsmCmdST!\n");
									return(ret);
								}
								break;
							default:
	...
	else if (enabled == 2)
	{
		...
		if(strlen(symmetricKeyDB.activeDate) >0)
		{
			...
			{
                	        UnionUserErrLog("in UnionDealServiceCodeE110:: activeDate[%s] error!\n",symmetricKeyDB.activeDate);
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE110:: activeDate can not be null!\n");
	...
	if (isRemoteKeyOperate)		// 远程密钥操作
	{
		...
		{
			UnionUserErrLog("in UnionDealServiceCodeE110:: UnionReadRemoteSysIDAndAppID!\n");
			return(ret);	
		}
	...
	if (isRemoteDistributeKey)	// 远程分发密钥
	{
		...
		if (mode == 1)	// 生成密钥
		{
			...
			{
				UnionUserErrLog("in UnionDealServiceCodeE110:: UnionSetRequestRemoteXMLPackageValue[%s][%s]!\n","body/keyValue",keyByRKM);
				return(ret);
			}
	...
	if (strlen(symmetricKeyDB.keyName) > 0)
	{
		if (mode == 0 || mode == 2)	// 不生成密钥
		{
			...
			{
				UnionUserErrLog("in UnionDealServiceCodeE110:: UnionCreateSymmetricKeyDB,keyName[%s]!\n",symmetricKeyDB.keyName);
				return(ret);	
			}
		}
		else
		{
			...
			{
				UnionUserErrLog("in UnionDealServiceCodeE110:: UnionGenerateSymmetricKeyDBRec,keyName[%s]!\n",symmetricKeyDB.keyName);
				return(ret);	
			}
		}
	}
	...
	if ((exportFlag != 0 && isRemoteKeyOperate == 0)|| (isRemoteKeyOperate && isApplyFlag == 1))
	{
		if (exportLmkKey[0] == '1' && !isRemoteKeyOperate)
		{
			...
			{
				UnionUserErrLog("in UnionDealServiceCodeE110:: UnionSetResponseXMLPackageValue[%s]!\n","body/keyValue2");
				return(ret);
			}
		}
