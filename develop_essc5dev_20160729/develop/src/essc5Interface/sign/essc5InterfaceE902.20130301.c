...
功能描述:	编制裸签(880)
***************************************/
int UnionDealServiceCodeE902(PUnionHsmGroupRec phsmGroupRec)
{
	int	ret;
	char	orgData[8192];
	...
	{
		UnionUserErrLog("in UnionDealServiceCodeE902:: UnionReadRequestXMLPackageValue[%s]!\n","head/orgData");
		return(orgDataLen);
	}
	...
	*/
	...
	{
		UnionUserErrLog("in UnionDealServiceCodeE902:: signFlag[%d] error!\n",signFlag);
		return(errCodeParameter);
	}
	...
	switch(phsmGroupRec->hsmCmdVersionID)
	{
		case conHsmCmdVerSRJ1401StandardHsmCmd:
			if ((ret = UnionSRJ1401CmdCM880((unsigned char *)bcdOrgData, orgDataLen, appName, algFlag, signData, &signDataLen)) < 0)
			{
				UnionUserErrLog("in UnionDealServiceCodeE902:: UnionSRJ1401CmdCM880,ret = [%d]!\n",ret);
				return(ret);
			}
			break;
		default:
			UnionUserErrLog("in UnionDealServiceCodeE902:: phsmGroupRec->hsmCmdVersionID[%d]\n",phsmGroupRec->hsmCmdVersionID);
	...
	if((ret = UnionSetResponseXMLPackageValue("body/signData",(char *)baseSignData)) < 0)
        {
                UnionUserErrLog("in UnionDealServiceCodeE902:: UnionSetResponseXMLPackageValue[%s]sigData[%s]!\n","body/signData", baseSignData);
                return(ret);
        }
