...
char *union_wide_to_escape_string(wide_char)
  zwchar wide_char;
{
  int i;
  zwchar w = wide_char;
...
char *union_wide_to_local_string(wide_string, escape_all)
  ...
  for (i = 0; i < wsize; i++) {
    ...
    if (escape_all) {
      if (b == 1 && (uch)buf[0] <= 0x7f) {
        ...
        char *escape_string = union_wide_to_escape_string(wide_string[i]);
...
/* UTF-8 to local */
char *union_utf8_to_local_string(utf8_string, escape_all)
  ZCONST char *utf8_string;
  int escape_all;
{
  zwchar *wide = union_utf8_to_wide_string(utf8_string);
  char *loc = union_wide_to_local_string(wide, escape_all);
  free(wide);
  return loc;
}
...
zwchar *union_utf8_to_wide_string(utf8_string)
  ZCONST char *utf8_string;
{
  int wcount;
  zwchar *wide_string;
