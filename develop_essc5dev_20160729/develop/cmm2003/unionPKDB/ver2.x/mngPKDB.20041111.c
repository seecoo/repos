...
int UnionHelp()
{
	...
	printf("	reload  reload pkDB into shared memory\n");
	printf("	connect Connect to pkDB\n");
...
int EditHelp()
{
	...
	printf("	passive print passive pks.\n");
	return(0);
}
...
int InsertPK(int argc,char *argv[],short confirmRequired)
{
	...
	if (argc > 3)
	{
		...
		aschex_to_bcdhex(argv[3],strlen(argv[3]),gpk.value);
...
int ReadPK(int argc,char *argv[])
{
	...
	for (i = 0; i < argc; i++)
	{
		...
		if ((ret = UnionReadPKFromKeyDBAnyway(&gpk)) < 0)
			printf("UnionReadPKFromKeyDB [%s] Error! ret = [%d]\n",gpk.fullName,ret);
		else
			UnionPrintPK(&gpk);
	}
...
int EditUpdate()
{
	...
	if (!UnionConfirm("Are you sure to update this des key?"))
		return(0);
	if ((ret = UnionUpdatePKInKeyDB(&gpk)) < 0)
		printf("UnionUpdatePKInKeyDB [%s] Error! ret = [%d]\n",gpk.fullName,ret);
	else
...
int EditRead()
{
	...
	if ((ret = UnionReadPKFromKeyDB(&gpk)) < 0)
		printf("UnionReadPKFromKeyDB [%s] Error! ret = [%d]\n",gpk.fullName,ret);
	else
		UnionPrintPK(&gpk);
	return(ret);
...
int UpdatePK(int argc,char *argv[])
{
	...
	if (argc > 1)
	{
		...
		aschex_to_bcdhex(argv[1],strlen(argv[1]),gpk.value);
...
int UpdatePKPassiveDate(int argc,char *argv[])
{
	int		ret;
...
int main(int argc,char **argv)
{
	...
	if ((ptaskInstance = UnionCreateTaskInstance(UnionTaskActionBeforeExit,"mngPKDB")) == NULL)
	{
	...
	gpk.length = con1024RSAPair;
	gpk.type = conSignature;
	//memset(gpk.value,'0',sizeof(gpk.value)-1);
	//memset(gpk.checkValue,'0',sizeof(gpk.checkValue)-1);
	UnionGetFullSystemDate(gpk.activeDate);
	memcpy(gpk.passiveDate,"21000101",8);
	...
	if (strcasecmp(argv[1],"PASSIVEDATE") == 0)
		return(UpdatePKPassiveDate(argc-2,&argv[2]));
