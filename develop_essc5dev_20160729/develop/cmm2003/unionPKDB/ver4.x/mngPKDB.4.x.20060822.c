...
int EditPK()
{
	char	*p;
	int	ret;	
	UnionFormDefaultPK(&gpk,"00.null.00","",conSignature,"");
loopEdit:
	p = UnionInput("\n\nCommand>");
	if (UnionIsValidPKFullName(p))
	{
		...
		{
			printf("%s not exists!\n",gpk.fullName);
			//UnionFormDefaultPK(&gpk,p,"",conSignature,"");
	...
	{
		if ((ret = UnionUnlockAllLockedPKInPKDB()) < 0)
			printf("UnionUnlockAllLockedPKInPKDB Failure! ret = [%d]\n",ret);
		else
			printf("UnionUnlockAllLockedPKInPKDB OK!\n");
	}
	if (strcasecmp(p,"ALLLOCKEDKEY") == 0)
	{
		if ((ret = UnionPrintAllLockedPKInPKDBToFile(stdout)) < 0)
			printf("UnionPrintAllLockedPKInPKDBToFile Failure! ret = [%d]\n",ret);
	}
	goto loopEdit;
}
...
int main(int argc,char **argv)
{
	...
	{
		if ((ret = UnionUnlockPKDB()) < 0)
			printf("UnionUnlockPKDB Failure! ret = [%d]\n",ret);
		else
			printf("UnionUnlockPKDB OK!\n");
		return(UnionTaskActionBeforeExit());
	}
	...
	*/
	...
	{
		if (UnionConfirm("Are you sure of reloading PKDB?"))
		{
