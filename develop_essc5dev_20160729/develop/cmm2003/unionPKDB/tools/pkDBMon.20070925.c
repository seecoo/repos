...
extern PUnionPKDB			pgunionPKDB;
extern PUnionPK			pgunionPK;
...
int RefreshPKDBWin(int interval)
{
	int		i,j;
	char		Buf[128];
	int		keyNum = 0;
	...
	for (keyNum = 0,i = 0;;)
	{
		...
		waddstr(pgContentWin,Buf);
		++i;
		for (;keyNum < pgunionPKDB->num; keyNum++)
		{
			keyNearOutdate = 0;
			keyIsLocked = 0;
			pkey = pgunionPK+keyNum;
			// 从有效期计算有效天数
...
int main(int argc,char *argv[])
{
	...
	return(UnionMonitorMain(interval,UnionGetApplicationName(),"密钥监控程序",UnionConnectPKDB,RefreshPKDBWin,UnionTaskActionBeforeExit));
}
