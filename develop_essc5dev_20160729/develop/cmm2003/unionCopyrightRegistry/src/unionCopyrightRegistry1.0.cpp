...
int UnionInitCopyrightRegistryStaticData(PUnionCopyrightRegistry pCopyrightRegistry,char *abbrProductName)
{
	...
	sprintf(fileName,"%s/UnionProductRegistry/%s/%s.Lisence.INI",
		getenv("HOME"),
		abbrProductName,
		abbrProductName);
...
int UnionOrganizeCopyrightRegistryDynamicData(PUnionCopyrightRegistry pCopyrightRegistry)
{
	...
	for (len = sizeof(pCopyrightRegistry->lisenceData)-1;;)
	{
		...
		printf("Input Lisence Data (= %d Characters)::\n",len);
		printf("++++++++++++++++\n");
		scanf("%s",tmpBuf);
		if (strlen(tmpBuf) != len)
			continue;
		strcpy(pCopyrightRegistry->lisenceData,tmpBuf);
		break;
	}
	...
	{
		memset(tmpBuf,0,sizeof(tmpBuf));
		printf("Input Lisenced User Number (1~1024)::\n");
		scanf("%d",&(pCopyrightRegistry->userNumber));
		if (UnionVerifyCopyrightRegistryUserNumber(pCopyrightRegistry->userNumber) < 0)
			continue;
		break;
	}
...
int UnionStoreCopyrightRegistry(PUnionCopyrightRegistry pCopyrightRegistry,char *nameOfProduct)
{
	...
	*/
	...
	memset(fileName,0,sizeof(fileName));
	sprintf(fileName,"%s/UnionProductRegistry/%s/Lisence%s.CFG",
			getenv("HOME"),
			nameOfProduct,
	...
	memset(cmd,0,sizeof(cmd));
	sprintf(cmd,"cp %s $HOME/etc/unionRegistry.CFG",fileName);
	printf("%s\n",cmd);
	system(cmd);
...
int UnionReadUserCopyrightRegistry(PUnionCopyrightRegistry pCopyrightRegistry)
{
	...
	memset(fileName,0,sizeof(fileName));
	sprintf(fileName,"%s/etc/unionRegistry.CFG",getenv("HOME"));
	if ((ret = UnionInitEnvi(fileName)) < 0)
	{
