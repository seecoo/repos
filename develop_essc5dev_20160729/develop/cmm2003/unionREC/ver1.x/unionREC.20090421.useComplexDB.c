...
#else
// 从complexDB中读取配置a wangk 20090717
int UnionReloadREC()
{
	int	ret = 0;
	int iCnt = 0;
	...
	{
		UnionUserErrLog("in UnionReloadREC:: MaxVarNumOfREC is too large\n");
		return (errCodeDefaultErrCode);
	}

	memset(tUnionREC, 0, 256 * sizeof(TUnionUnionREC));
...
int UnionGetMaxVarNumOfREC()
{
	int	ret = 0;
	int	num = 0;
	if ((ret=UnionReadUnionRECRecIntTypeFld("MaxNumOfVar", "varValue", &num)) < 0)
	{
		UnionUserErrLog("in UnionGetMaxVarNumOfREC:: UnionReadUnionRECRecIntTypeFld for [%s]\n!","MaxNumOfVar");
		return ret;
	}
...
int UnionReadLongTypeRECVar(char *varName)
{
	PUnionRECVar	pvar = NULL;
...
int UnionReadShortTypeRECVar(char *varName)
{
	PUnionRECVar	pvar = NULL;
...
int UnionReadDoubleTypeRECVar(char *varName)
{
	PUnionRECVar	pvar = NULL;
...
int UnionUpdateRECVar(char *varName,unsigned char *value)
{
	...
	char		fileName[512];
	int		ret;
        char            v_tmpbuf1[1024+1], v_tmpbuf2[1024+1], v_tmpbuf3[1024+1];
...
int UnionConvertRECVarTypeIntoTypeName(TUnionRECVarType type,char *nameOfType)
{
	...
	switch (type)
	{
		...
		case	conDouble:
			...
			break;
		default:
			UnionUserErrLog("in UnionConvertRECVarTypeIntoTypeName:: type [%d] error!\n",type);
			return(errCodeRECMDL_VarType);
	}
	return(0);
}
...
int UnionChargeFieldNameToDBFieldName(char *fldName,char *prefixFldName)
{
        ...
        if ((ptr = UnionReadStringTypeRECVar("prefixOfDBField")) == NULL)
