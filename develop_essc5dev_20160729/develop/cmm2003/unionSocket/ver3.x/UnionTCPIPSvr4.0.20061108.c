...
int UnionTCPIPServer(int port,char *ServerName __attribute__((unused)),int (*UnionTCPIPTaskServer)(),int (*UnionTaskActionBeforeExit)())
{
	...
	for (;;)
	{
		...
		if (setsockopt(sckinstance,SOL_SOCKET,SO_LINGER,(char *)&Linger,sizeof(Linger)) != 0)
		{
			...
			close(sckinstance);
			//return(errCodeUseOSErrCode);	// 2006/10/23，屏蔽了这句，改为continue
			continue;
		}
