...
//	3.0与之前的版本比，代码完全不同。
//	20040406, 在3.0基础上升级为3.1，修改了CreateInstance函数，
//		3.0的函数在重置任务类型时，未同时重置日志

//	2004/06/18 升级为3.2

//	2004/06/25，王纯军，由3.2升级为4.0
//	在UnionCreateTaskInstance中增加了验证版本信息的功能
...
#include "unionLisence.h"	// Added by Wolfgang Wang, 2004/06/25
#include "UnionLog.h"
...
int UnionPrintTaskInstanceToFile(PUnionTaskInstance ptaskInstance,FILE *fp)
{
	...
	fprintf(fp,"%012x,",ptaskInstance->plogFile);
...
PUnionTaskInstance UnionCreateTaskInstance(int (*UnionTaskActionBeforeExit)(),char *fmt,...)
{
	...
	// Added by Wolfgang Wang, 2004/06/25
	if ((ret = UnionVerifyLisenceCode()) < 0)
	{
		UnionUserErrLog("in UnionCreateTaskInstance:: UnionVerifyLisenceCode!\n");
		return(NULL);
	}	
	// End of addition of 2004/06/25
