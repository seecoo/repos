...
int UnionReloadLogFileTBL()
{
	...
	if (pgunionLogFileTBL == NULL)
	{
		UnionUserErrLog("in UnionReloadLogFileTBL:: pgunionLogFileTBL is NULL!\n");
		return(errCodeLogMDL_NotConnected);
	}
...
int UnionPrintLogFileTBL()
{
	...
	for (i = 0; i < pgunionLogFileTBL->maxLogFileNum; i++)
	{
		if (UnionPrintLogFile(pgunionLogFile+i) < 0)
			continue;
		num++;
		if ((num != 0) && (num % 23 == 0))
		{
	...
	if (UnionIsQuit(UnionPressAnyKey("Press any key to continue or exit/quit to exit...")))
		return(0);
	UnionPrintSharedMemoryModuleToFile(pgunionLogFileMDL,stdout);
	return(0);
}
