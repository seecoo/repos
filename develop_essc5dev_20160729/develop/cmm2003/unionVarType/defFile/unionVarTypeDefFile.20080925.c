...
int UnionExistsVarTypeDefInDefaultDefFile(char *nameOfType)
{
	...
	return(UnionExistsVarTypeDefInSpecFile(fileName,nameOfType));
}	
...
int UnionReadVarTypeDefFromDefaultFile(char *nameOfType,PUnionVarTypeDef pdef)
{
	...
	UnionGetDefaultFileNameOfVarTypeTagDef(nameOfType,fileName);
	return(UnionReadVarTypeDefFromSpecFile(fileName,nameOfType,pdef));
}
...
int UnionWriteVarTypeDefIntoDefaultFile(PUnionVarTypeDef pdef)
{
	...
	if (pdef == NULL)
		return(errCodeParameter);
	UnionGetDefaultFileNameOfVarTypeTagDef(pdef->nameOfType,fileName);
	return(UnionWriteVarTypeDefIntoSpecFile(pdef,fileName));
}
