...
int UnionReadSimpleTypeDefFromDefaultDefFile(char *nameOfType,PUnionSimpleTypeDef pdef)
{
        ...
        strcat(condition, "|");
        /* 从表里取出一条record */
        if ((ret = UnionSelectUniqueObjectRecordByPrimaryKey("simpleTypeDef", condition, record, sizeof(record)))<0)

        {
                UnionUserErrLog("in UnionReadSimpleTypeDefFromDefaultDefFile:: UnionReadSimpleTypeDefFromDefaultDefFile!\n");
                return(ret);
        }
