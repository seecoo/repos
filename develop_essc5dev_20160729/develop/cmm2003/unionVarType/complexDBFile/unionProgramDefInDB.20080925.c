...
int UnionReadProgramDefFromDefaultFile(char *nameOfProgram,PUnionProgramDef pdef)
{
	char       record[1024+1];
        ...
        strcpy(condition, "nameOfProgram=");
        strcat(condition, nameOfProgram);
        strcat(condition, "|");
        /* 从表里取出一条record */
        if ((ret = UnionSelectUniqueObjectRecordByPrimaryKey("program", condition, record, sizeof(record)))<0)

        {
                UnionUserErrLog("in UnionReadProgramDefFromDefaultFile:: UnionReadProgramDefFromDefaultFile!\n");
                return(ret);
        }
        ...
        /* 给版本信息赋值 */
        UnionReadRecFldFromRecStr(record, strlen(record), "version", pdef->version, sizeof (pdef->version));
