...
int UnionHelp()
{
	printf("Usage:: genTBLSQL [tableName|all|tableList]\n");
	return(0);
}
...
int main(int argc,char *argv[])
{
	...
	if (argc == argvIndex + 1)
	{
		...
		if (strcmp(tmpBuf,"ALL") == 0)
			return(UnionCreateAllObjectDefSQLFromTBLDefInTableList(stdout));
	}
	for (index = argvIndex; index < argc; index++)
	{
		strcpy(tmpBuf,argv[index]);
		if ((ret = UnionCreateObjectDefSQLFromTBLDefInTableList(tmpBuf,stdout)) < 0)
		{
			printf("UnionCreateObjectDefSQLFromTBLDefInTableList [%s] failure! ret = [%d]\n",tmpBuf,ret);
			continue;
		}
		else
			printf("UnionCreateObjectDefSQLFromTBLDefInTableList [%s] ok!\n",tmpBuf);
	}
	if (argc > argvIndex + 1)
		return(0);
