...
#include "unionUnionKeyDBBackupServer.h"
#endif
...
int UnionGetNameOfKeyDBBackupServer(char *fileName)
{
	sprintf(fileName,"unionKeyDBBackupServer");
	return(0);
}
#endif
...
int UnionLoadKeyDBBackupServerIntoMemory()
{
	...
	int								num = 0;
	char							szVarName[16];
	TUnionUnionKeyDBBackupServer	tKeyDBBackupSer;
	...
	memset(&tKeyDBBackupSer, 0, sizeof(TUnionUnionKeyDBBackupServer));
	if( (ret = UnionReadUnionKeyDBBackupServerRec("active", &tKeyDBBackupSer)) < 0 )
	{
		UnionUserErrLog("in UnionLoadKeyDBBackupServerIntoMemory:: UnionReadUnionKeyDBBackupServerRec for [active]\n");
		return (ret);
	}
	pgunionKeyDBBackupServer->active = atoi(tKeyDBBackupSer.propertyField2);
	
	memset(&(pgunionKeyDBBackupServer->myself),0,sizeof(pgunionKeyDBBackupServer->myself));
	memset(&tKeyDBBackupSer, 0, sizeof(TUnionUnionKeyDBBackupServer));
	if( (ret = UnionReadUnionKeyDBBackupServerRec("myself", &tKeyDBBackupSer)) < 0 )
	{
		UnionUserErrLog("in UnionLoadKeyDBBackupServerIntoMemory:: UnionReadUnionKeyDBBackupServerRec for [myself]\n");
		return (ret);
	}
	
	if( !UnionIsValidIPAddrStr(tKeyDBBackupSer.propertyField2) )
	{
		UnionUserErrLog("in UnionLoadKeyDBBackupServerIntoMemory:: get ipAddr for [myself]!\n");
		return (errCodeKeyDBBackuperMDL_ConfError);
	}
	strcpy(pgunionKeyDBBackupServer->myself.ipAddr, tKeyDBBackupSer.propertyField2);
	if( tKeyDBBackupSer.propertyField3 == NULL )
	{
		UnionUserErrLog("in UnionLoadKeyDBBackupServerIntoMemory:: get port for [myself]!\n");
		return (errCodeKeyDBBackuperMDL_ConfError);
	}
	pgunionKeyDBBackupServer->myself.port = atoi(tKeyDBBackupSer.propertyField3);
	UnionLog("in UnionLoadKeyDBBackupServerIntoMemory:: port = [%s]\n", tKeyDBBackupSer.propertyField3);
	...
	if( (ret = UnionReadUnionKeyDBBackupServerRec("brother", &tKeyDBBackupSer)) > 0 )
	{
		if( UnionIsValidIPAddrStr(tKeyDBBackupSer.propertyField2) )
		{
			strcpy(pgunionKeyDBBackupServer->brother[pgunionKeyDBBackupServer->realNum].ipAddr, tKeyDBBackupSer.propertyField2);
		
			if( tKeyDBBackupSer.propertyField3 != NULL )
			{
				pgunionKeyDBBackupServer->brother[pgunionKeyDBBackupServer->realNum].port = atoi(tKeyDBBackupSer.propertyField3);
			
				if( tKeyDBBackupSer.propertyField4 != NULL )
				{
					pgunionKeyDBBackupServer->brother[pgunionKeyDBBackupServer->realNum].active = atoi(tKeyDBBackupSer.propertyField4);
				}
				else
				{
					pgunionKeyDBBackupServer->brother[pgunionKeyDBBackupServer->realNum].active = pgunionKeyDBBackupServer->active;
	...
	for( num = 0; (num < conMaxNumOfKeyDBBackuper) && (pgunionKeyDBBackupServer->realNum < conMaxNumOfKeyDBBackuper); num++ )
	{
		...
		sprintf(szVarName,"brother%02d",num);
		if( (ret = UnionReadUnionKeyDBBackupServerRec(szVarName, &tKeyDBBackupSer)) > 0 )
		{
			UnionUserErrLog("in UnionLoadKeyDBBackupServerIntoMemory:: UnionReadUnionKeyDBBackupServerRec for [%s]\n", szVarName);
			continue;
		}

		if( !UnionIsValidIPAddrStr(tKeyDBBackupSer.propertyField2) )
		{
			UnionUserErrLog("in UnionLoadKeyDBBackupServerIntoMemory:: get ipAddr for [%s]!\n", tKeyDBBackupSer.propertyField1);
			continue;
		}
		strcpy(pgunionKeyDBBackupServer->brother[pgunionKeyDBBackupServer->realNum].ipAddr, tKeyDBBackupSer.propertyField2);
