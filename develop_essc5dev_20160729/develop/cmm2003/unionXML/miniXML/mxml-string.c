  ...
  while (*format)
  {
	...
	*/
	...
	case 'g' :
		...
		memcpy(bufptr, temp, (size_t)(bufend - bufptr) + 1);
		bufptr = bufend;
	      }
	      else
	      {
		//strcpy(bufptr, temp);
		memcpy(bufptr,temp,strlen(temp) + 1);
		bufptr += strlen(temp);
	      }
	    }
	    break;
	...
	case 's' : /* String */
		...
		memcpy(bufptr + width - slen, s, (size_t)slen + 1);
	      }
               ...
               va_list    ap)		/* I - Pointer to additional arguments */
{
  ...
  char		*buffer,		/* String buffer */
		temp[32];		/* Small buffer for first vsnprintf */
