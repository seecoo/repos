...
mxmlGetCDATA(mxml_cache_t *mxml_cache,mxml_node_t *node)		/* I - Node to get */
{
 /*
  * Range check input...
  */
...
//      strncmp(node->value.element.name, "![CDATA[", 8))
  ...
  */
  ...
  return (mxml_cache->buffer + node->value.element.name_offset + 8);
}
...
mxmlGetElement(mxml_cache_t *mxml_cache,mxml_node_t *node)	/* I - Node to get */
{
 /*
  * Range check input...
  */
  ...
  */
  ...
  return (mxml_cache->buffer + node->value.element.name_offset);
}
...
mxmlGetOpaque(mxml_cache_t *mxml_cache,mxml_node_t *node)	/* I - Node to get */
{
 /*
  * Range check input...
  */
...
//    return (node->value.opaque);
    return (mxml_cache->buffer + node->value.opaque_offset);
  else if (node->type == MXML_ELEMENT &&
           node->child &&
	   node->child->type == MXML_OPAQUE)
//    return (node->child->value.opaque);
    return (mxml_cache->buffer + node->child->value.opaque_offset);
  else
    return (NULL);
}
...
mxmlGetText(mxml_cache_t *mxml_cache,mxml_node_t *node,		/* I - Node to get */
...
//    return (node->value.text.string);
    return (mxml_cache->buffer + node->value.text.string_offset);
  }
  else if (node->type == MXML_ELEMENT &&
           node->child &&
...
//    return (node->child->value.text.string);
    return (mxml_cache->buffer + node->child->value.text.string_offset);
  }
  else
  {
    if (whitespace)
