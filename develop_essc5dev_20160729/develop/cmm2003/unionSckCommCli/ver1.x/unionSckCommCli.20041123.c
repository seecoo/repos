...
#ifdef _selfBalanceSckSvr_
int				gunionCurrentSvrIndex = -1;
#endif
...
loop:
	switch (gunionCurrentSvrIndex)
	{
		case	-1:
		case	1:
			gunionCurrentSvrIndex = 0;
			break;
		case	0:
			gunionCurrentSvrIndex = 1;
			break;
		default:
			gunionCurrentSvrIndex = 0;
			break;
	...
	retryTimes++;
	if (pcommSvrDef->sckSvrDef[gunionCurrentSvrIndex].status == conSckSvrNormal)
		return(&(pcommSvrDef->sckSvrDef[gunionCurrentSvrIndex]));
	else
	{
		if (retryTimes >= 2)
		{
			pcommSvrDef->sckSvrDef[gunionCurrentSvrIndex].status = conSckSvrNormal;
			return(&(pcommSvrDef->sckSvrDef[gunionCurrentSvrIndex]));
		}
		else
			goto loop;
...
PUnionSckSvrDef UnionSelectIdleSckSvr(PUnionCommSvrDef pcommSvrDef)
{
	...
	if (pcommSvrDef->sckSvrDef[0].successTimes <= pcommSvrDef->sckSvrDef[1].successTimes)
	{
		if (pcommSvrDef->sckSvrDef[0].status == conSckSvrNormal)
			return(&(pcommSvrDef->sckSvrDef[0]));
		else
...
selectIdle:	
	...
	else
	{
		...
		{
			UnionSuccessLog("in UnionCommSvrConnHotBackupService:: UnionSelectIdleSckSvr!\n");
			return(errCodeSckCommMDL_NoWorkingServer);
		}
	}
	//UnionLog("in UnionCommSvrConnHotBackupService:: psckSvrDef = [%x]\n",psckSvrDef);
//reinit:
	if ((psckCommSvrConn->psckCommConn[0] != NULL) && (psckCommSvrConn->psckCommConn[0]->psckSvrDef == psckSvrDef))
			index = 0;
	else if ((psckCommSvrConn->psckCommConn[1] != NULL) && (psckCommSvrConn->psckCommConn[1]->psckSvrDef == psckSvrDef))
			index = 1;
	if (index < 0)
	{
		for (i = 0; i < 2; i++)
		{
			...
			{
				UnionUserErrLog("in UnionCommSvrConnBalanceService:: UnionCreateSckSvrConn!\n");
				psckSvrDef->status = conSckSvrAbnormal;
				retryTimes++;
				goto selectIdle;
			}
			...
			{
				index = i;
				UnionUserErrLog("in UnionCommSvrConnBalanceService:: connect create OK [%s] [%d] index = [%d]\n",psckSvrDef->ipAddr,psckSvrDef->port,index);
				break;
			}
		}
	}
	...
	{
		//UnionUserErrLog("in UnionCommSvrConnBalanceService:: UnionCommWithSckSvr!\n");
		//if (ret == errSocketError)
		{
			UnionUserErrLog("in UnionCommSvrConnBalanceService:: errSocketError index = [%d] psckSvr = [%p] [%s] [%d]!\n",index,psckSvrDef,psckSvrDef->ipAddr,psckSvrDef->port);
