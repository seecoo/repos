...
int UnionWritingLockDesKeyTBL()
{
	...
	{					// 表已被锁住
		UnionUserErrLog("in UnionWritingLockDesKeyTBL:: isWritingLocked = [%d] readingLocks = [%d]!\n",pgunionDesKeyDB->isWritingLocked,pgunionDesKeyDB->readingLocks);	// 2010-6-5，王纯军增加此句
	...
	for (index = 0; index <  pgunionDesKeyDB->num; index++)
	{
		...
		{
			pgunionDesKeyDB->isWritingLocked = 0;
			UnionUserErrLog("in UnionWritingLockDesKeyTBL:: [%s] is writing-locked!\n",(pgunionDesKey+index)->fullName);	// 2010-6-5，王纯军增加此句
...
retry:
	...
	if (pgunionDesKeyDB->isWritingLocked > 0)			// 表已被锁住
	{
		if (retryTimes == -2)
		{
			if ((retryTimes = UnionReadIntTypeRECVar("applyReadingLockRetryTimes")) > 10000)
				retryTimes = 10000;
			if (retryTimes < 0)
				retryTimes = 50;
		}
		if (sleepTimes == -2)
		{
			if ((sleepTimes = UnionReadIntTypeRECVar("applyReadingLockSleepTimes")) < 100 )
				sleepTimes = 100;
			if (sleepTimes > 1000)
				sleepTimes = 1000;
		}
...
long UnionGetMaxKeyNumFromKeyDBFile()
{
	...
	// added by Wolfgang Wang,2009/7/8
	if ((maxNum = UnionReadIntTypeRECVar("maxNumOfDesKey")) > 0)
	{
		return(maxNum);
	}
	// end of addition of 2009/7/8
...
retry:
        ...
        memset(&keyDB,0,sizeof(keyDB));
        if (fread(&keyDB,sizeof(keyDB),1,fp) != 1)
        {
                fclose(fp);
                retry--;
                if (retry >= 0)
...
int UnionInsertDesKeyIntoKeyDB(PUnionDesKey pkey)
{
	...
	{
		UnionUserErrLog("in UnionInsertDesKeyIntoKeyDB:: UnionIsValidDesKey [%s],type=[%d],length=[%d],value=[%s],checkValue=[%s]!\n", pkey->fullName,pkey->type,pkey->length,pkey->value,pkey->checkValue);
		return(errCodeParameter);
	}
	if ((ret = UnionWritingLockDesKeyTBL()) < 0)
	{
...
int UnionSynchronizeSpecifiedDesKeyToBackuper(char *fullName)
{
	...
	if ((ret = UnionWritingLockDesKey(pgunionDesKey+index)) < 0)
	{
		UnionUserErrLog("in UnionSynchronizeAllDesKeyToBackuper:: UnionWritingLockDesKey !\n");
		return(ret);
	}
...
// 2009-7-1 wangk
int UnionPrintDesKeyGroupInKeyDBToFileForEssc(PUnionDesKeyQueryCondition pcondition,FILE *fp)
{
	int	ret = 0;
	long	i = 0;
	long	totalNum = 0;
	...
	if ((ret = UnionReadingLockDesKeyTBL()) < 0)
	{
		UnionUserErrLog("in UnionPrintDesKeyGroupInKeyDBToFileForEssc:: UnionReadingLockDesKeyTBL!\n");
		return(ret);
	}
...
writeNow:
		...
		memset(nameOfType,0,sizeof(nameOfType));
		UnionGetNameOfDesKeyType((pgunionDesKey+i)->type,nameOfType);
		fprintf(fp, "fullName=%s|value=%s|checkValue=%s|length=%d|type=%s|activeDate=%s|"
					"maxEffectiveDays=%ld|maxUseTimes=%ld|useTimes=%ld|lastUpdateTime=%ld|"
					"container=%s|oldVerEffective=%d|oldValue=%s|oldCheckValue=%s|"
					"windowBetweenKeyVer=%d|isWritingLocked=%d|\n",
			(pgunionDesKey+i)->fullName, (pgunionDesKey+i)->value, (pgunionDesKey+i)->checkValue,
			UnionGetDesKeyBitsLength((pgunionDesKey+i)->length), nameOfType,
			(pgunionDesKey+i)->activeDate, (pgunionDesKey+i)->maxEffectiveDays, (pgunionDesKey+i)->maxUseTimes,
			(pgunionDesKey+i)->useTimes, (pgunionDesKey+i)->lastUpdateTime, (pgunionDesKey+i)->container,
			(pgunionDesKey+i)->oldVerEffective, (pgunionDesKey+i)->oldValue, (pgunionDesKey+i)->oldCheckValue,
			(pgunionDesKey+i)->windowBetweenKeyVer, (pgunionDesKey+i)->isWritingLocked);
		if ((fp == stdout) && (totalNum % 23 == 0) && (totalNum != 0))
		{
			printf("Press any key to continue...\n");
			getchar();
...
// 2009-7-1 wangk
int UnionPrintDesKeyGroupInKeyDBToSpecFileForEssc(PUnionDesKeyQueryCondition pcondition,char *fileName)
{
        FILE    *fp;
        int     ret;
        ...
        else
        {
                if ((fp = fopen(fileName,"w")) == NULL)
                {
                        UnionSystemErrLog("in UnionPrintDesKeyGroupInKeyDBToSpecFileForEssc:: fopen [%s]\n",fileName);
                        return(errCodeUseOSErrCode);
                }
        }
        ret = UnionPrintDesKeyGroupInKeyDBToFileForEssc(pcondition,fp);
        if (fp != stdout)
                fclose(fp);
        return(ret);
}
...
// 2007/11/29, Added by Wolfgang Wang
long UnionCompareAllDesKeyValueInKeyDBWithBrothersToFp(FILE *fp)
{
	int	ret;
	long	i;
	long	checkErrNum = 0;
	...
	for (i = 0; i < pgunionDesKeyDB->num; i++)
	{
		if ((ret = UnionCompareDesKeyValueWithBrothers((pgunionDesKey+i)->fullName,(pgunionDesKey+i)->value)) < 0)
		{
			...
			fprintf(outFp,"check Err::[%s]\n",(pgunionDesKey+i)->fullName);
			checkErrNum++;
		}
		else
		{
			UnionAuditLog("check OK ::[%s]\n",(pgunionDesKey+i)->fullName);
			fprintf(outFp,"check OK ::[%s]\n",(pgunionDesKey+i)->fullName);
		}
	}
	ret = checkErrNum;
...
long UnionCompareAllDesKeyValueInKeyDBWithBrothers(FILE *fp)
{
	return(UnionCompareAllDesKeyValueInKeyDBWithBrothersToFp(stdout));
}
...
int UnionCheckAllUnsameDesKeyInImageAndFileToFp(FILE *fp)
{
	return(0);
}
