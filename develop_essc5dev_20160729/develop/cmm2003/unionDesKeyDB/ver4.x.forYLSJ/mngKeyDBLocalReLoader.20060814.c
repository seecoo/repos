...
int UnionCheckDesKeyDB(char *fileName,char *lastUpdatedTime)
{
	...
	{
		UnionSystemErrLog("in UnionCheckDesKeyDB:: UnionFsize [%s],size[%d]\n",fileName,fsize);
	...
	if (fsize != presize)
	{
		printf("in UnionCheckDesKeyDB:: file[%s] is err! fsize[%d],presize[%d] \n",fileName,fsize,presize);
		
		UnionSystemErrLog("in UnionCheckDesKeyDB:: file[%s] is err! fsize[%d],presize[%d] \n",fileName,fsize,presize);
		return(errCodeUseOSErrCode);
	}
	return 0;
}
