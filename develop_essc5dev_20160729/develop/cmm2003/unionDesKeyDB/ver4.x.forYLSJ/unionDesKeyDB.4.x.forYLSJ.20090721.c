...
#include "unionDesKeyDB.forYLSJ.h"
...
// Added By HuangBaoxin, 20090722, 银联数据将1.0版本的密钥加载到共享内存
#ifndef	_UnionDesKey_1_x_YLSJ_
#define	_UnionDesKey_1_x_YLSJ_
#endif
#include "unionDesKey1x_3x.h"
// End Added By HuangBaoxin, 20090722
...
#else
	sprintf(fileName, "%s/etc/unionKeyDB/unionDesKeyDB.KDB", getenv("HOME"));
#endif
	return(0);
}
...
int UnionMirrorDesKeyDBIntoDisk(PUnionDesKeyDB pdesKeyDB,PUnionDesKey pkey)
{
	...
	{
		if (( ret = UnionSaveDesKey3x(pkey+i)) < 0)
		{
			UnionUserErrLog("in UnionMirrorDesKeyDBIntoDisk::keyName[%s]\n",(pkey+i)->fullName);
			fclose(fp);
			return ret;
		}
	}
...
#else
	if ((ret = UnionLoadDesKey1xIntoMemory(pgunionDesKeyDB)) < 0)
	{
		UnionUserErrLog("in UnionLoadDesKeyDBIntoMemory:: UnionLoadDesKey1xIntoMemory!\n");
		goto exitRightNow;		
	}
	
	if ((ret = UnionLoadOldDesKey1xIntoMemory(pgunionDesKeyDB)) < 0)
	{
		UnionUserErrLog("in UnionLoadDesKeyDBIntoMemory:: UnionLoadOldDesKey1xIntoMemory!\n");
		goto exitRightNow;		
	}
#endif
...
#else
	if ((ret = UnionSaveDesKey3x(pkey)) < 0)
		UnionUserErrLog("in UnionInsertDesKeyIntoKeyDB:: UnionSaveDesKey3x!\n");
#endif
...
#else
	if ((ret = UnionDeleteDesKey3x(&key)) < 0)
		UnionUserErrLog("in UnionDeleteDesKeyFromKeyDB:: UnionDeleteDesKey3x!\n");	
#endif
...
#else
	//key.oldVerEffective = 1;
	if ((ret = UnionSaveDesKey3x(&key)) < 0)
		UnionUserErrLog("in UnionUpdateDesKeyValueInKeyDB:: UnionSaveDesKey3x!\n");
#endif
...
int UnionDesKeyDBOperationByValueType(TUnionKeyDBOperation operation,PUnionDesKey pdesKey, int valueType)
{
	...
	switch (operation)
	{
		...
		case	conUpdateRecordAllFlds:
			...
			{
				if ((ret = UnionUpdateAllDesKeyFieldsInKeyDBByValueType(pdesKey, valueType)) < 0)
					UnionUserErrLog("in UnionDesKeyDBOperationByValueType:: UnionUpdateAllDesKeyFieldsInKeyDBByValueType [%s]\n",pdesKey->fullName);
				else
					UnionSuccessLog("in UnionDesKeyDBOperationByValueType:: UnionUpdateAllDesKeyFieldsInKeyDBByValueType [%s] OK\n",pdesKey->fullName);
			}
			break;
		case	conDeleteRecord:
			if ((ret = UnionDeleteDesKeyFromKeyDB(pdesKey->fullName)) < 0)
...
#else
	if ((ret = UnionSaveDesKey3x(&desKey)) < 0)
		UnionUserErrLog("in UnionUpdateDesKeyValueInKeyDB:: UnionSaveDesKey3x!\n");
#endif
...
int UnionUpdateAllDesKeyFieldsInKeyDBByValueType(PUnionDesKey pkey, int valueType)
{
	...
	if ((ret = UnionApplyKDBService(conUpdateRecordAllFlds,conIsDesKeyDB,(unsigned char *)(pkey),sizeof(*pkey))) < 0)
	{
		UnionUserErrLog("in UnionUpdateAllDesKeyFieldsInKeyDBByValueType:: UnionApplyKDBService !\n");
		return(ret);
	}
	return(ret);
#endif
	...
	if (!UnionIsValidDesKey(pkey))
	{
		UnionUserErrLog("in UnionUpdateAllDesKeyFieldsInKeyDBByValueType:: UnionIsValidDesKey [%s]\n",pkey->fullName);
		return(errCodeParameter);
	}
	
	if ((index = UnionFindDesKeyPosInKeyDB(pkey->fullName)) < 0)
	{
		UnionUserErrLog("in UnionUpdateAllDesKeyFieldsInKeyDBByValueType:: UnionFindDesKeyPosInKeyDB [%s]!\n",pkey->fullName);
		return((int)index);
	}

	if ((ret = UnionWritingLockDesKey(pgunionDesKey+index)) < 0)
	{
		UnionUserErrLog("in UnionUpdateAllDesKeyFieldsInKeyDBByValueType:: UnionWritingLockDesKey !\n");
		return(ret);
	}
...
#ifndef _UnionDesKey_1_x_YLSJ_
	if ((ret = UnionMirrorDesKeyDBIntoDisk(pgunionDesKeyDB,pgunionDesKey)) < 0)
		UnionUserErrLog("in UnionUpdateAllDesKeyFieldsInKeyDBByValueType:: UnionMirrorDesKeyDBIntoDisk!\n");
#else
	//pkey->oldVerEffective = 1;
	//if ((ret = UnionSaveDesKey3x(pkey)) < 0)
	if ((ret = UnionSaveDesKey3xByValueType(pkey, valueType)) < 0)
		UnionUserErrLog("in UnionUpdateAllDesKeyFieldsInKeyDBByValueType:: UnionSaveDesKey3xByValueType!\n");
#endif
