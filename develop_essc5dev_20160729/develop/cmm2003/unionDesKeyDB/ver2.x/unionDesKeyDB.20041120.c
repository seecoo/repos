...
long UnionSynchronizeAllDesKeyToBackuper()
{
	...
	if ((pgunionDesKey == NULL) || (pgunionDesKeyDB == NULL))
	{
		UnionUserErrLog("in UnionSynchronizeAllDesKeyToBackuper:: pkey is NULL!\n");
		return(-1);
	}
...
long UnionSynchronizeDesKeyOfSpecifiedAppToBackuper(char *appName)
{
	...
	if ((pgunionDesKey == NULL) || (pgunionDesKeyDB == NULL))
	{
		UnionUserErrLog("in UnionSynchronizeDesKeyOfSpecifiedAppToBackuper:: pkey is NULL!\n");
		return(-1);
	}
...
int UnionDesKeyDBOperation(TUnionKeyDBOperation operation,PUnionDesKey pdesKey)
{
	...
	if ((pgunionDesKey == NULL) || (pgunionDesKeyDB == NULL))
	{
		...
		{
			UnionUserErrLog("in UnionDesKeyDBOperation:: UnionConnectDesKeyDB Error!\n");
			return(ret);
		}
	}
	...
	switch (operation)
	{
		...
		case	conUpdateRecord:
			...
			{
				if ((ret = UnionUpdateDesKeyInKeyDB(pdesKey)) < 0)
					UnionUserErrLog("in UnionDesKeyDBOperation:: UnionUpdateDesKeyInKeyDB [%s]\n",pdesKey->fullName);
				else
					UnionSuccessLog("in UnionDesKeyDBOperation:: UnionUpdateDesKeyInKeyDB [%s] OK\n",pdesKey->fullName);
			}
			break;
