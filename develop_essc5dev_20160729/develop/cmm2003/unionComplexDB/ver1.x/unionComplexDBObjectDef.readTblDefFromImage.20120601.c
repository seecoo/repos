...
int UnionReadObjectDef(TUnionIDOfObject idOfObject,PUnionObject pobject)
{
	...
	// added 2012-05-09
	if((ptr = getenv("isReadTableDefFromDefFile")) == NULL || atoi(ptr) == 0)
	{
		if((iRet = UnionReadTableDefFromImage(idOfObject, pobject)) >= 0)
		{
			//UnionProgramerLog("in UnionReadObjectDef:: UnionReadTableDefFromImage read table[%s] def from image OK!!\n", idOfObject);
			return(iRet);
		}
	}
	// end of addition 2012-05-09
	...
	//UnionCloseRecFileHDLOfFileName(hFileHdl,caFileName);
	fclose(hFileHdl);
	UnionProgramerLog("in UnionReadObjectDef:: read table[%s] def from FILE[object/objDef/%s.def] OK!!\n", idOfObject, idOfObject);

insertImage:
	// added 2012-05-10
	if((ptr = getenv("isReadTableDefFromDefFile")) == NULL || atoi(ptr) == 0)
	{
		if((iRet = UnionInsertTableDefIntoImage(pobject)) < 0)
		{
			UnionUserErrLog("in UnionReadObjectDef:: UnionInsertTableDefIntoImage tableName = [%s] ret = %d!\n", idOfObject, iRet);
			//return(UnionSetUserDefinedErrorCode(iRet));
		}
	}
	// end of addition 2012-05-10
...
int UnionReadPrimaryKeyOfSpecObject(char *tblName,char *primaryKey)
{
	...
	// added 2012-07-03
	if((ret = UnionReadTablePrimaryKeyFromImage(tblName, primaryKey)) > 0)
	{
		return(0);
	}
	// end of addition 2012-07-03
