...
R_RSA_PUBLIC_KEY *publicKey;    /* RSA public key */
{
	...
	printf("\nData [%d] to decrypted by public key as follow:\n",inputLen);
	...
	{
		printf("in RSAPublicDecrypt:: step 1.\n");
		return(RE_LEN);
	}
	...
	if(status)
	{
		printf("in RSAPublicDecrypt:: step 2.[%d]\n",status);
		return(status);
	}
	
	printf("\nin RSAPublicDecrypt:: Len = [%d] Digest as follow:\n",pkcsBlockLen);
	for (i = 0; i < pkcsBlockLen; i++)
	{
		printf("%02x",pkcsBlock[i]);
	}
	printf("\n\n");
	...
	{
		printf("in RSAPublicDecrypt:: step 3.\n");
		return(RE_LEN);
	}
	...
	{
		printf("in RSAPublicDecrypt:: step 4.\n");
		return(RE_DATA);
	}
	...
	if(pkcsBlock[i++] != 0)
	{
		printf("in RSAPublicDecrypt:: step 5.\n");
		return(RE_DATA);
	}
	...
	*outputLen = modulusLen - i;
	...
	{
		printf("in RSAPublicDecrypt:: step 6.\n");
		return(RE_DATA);
	}
...
R_RSA_PRIVATE_KEY *privateKey;  /* RSA private key */
{
	...
	printf("\nData [%d] to encrypted by private key as follow:\n",inputLen);
	for (i = 0; i < inputLen; i++)
		printf("%02x",input[i]);
	printf("\n\n");
	...
	if(inputLen + 11 > modulusLen)
	{
		printf("in RSAPrivateEncrypt:: modulusLen error! modulusLen = [%d]\n",modulusLen);
		return (RE_LEN);
	}
	...
	*(pkcsBlock+1) = 1;
	...
	printf("\nin RSAPrivateEncrypt:: Digist Len = [%d] Buf as follow:\n",modulusLen);
...
*/
int RSAPublicEnc(output, outputLen, input, inputLen, publicKey)
unsigned char *output;          /* output block */
unsigned int *outputLen;        /* length of output block */
unsigned char *input;           /* input block */
unsigned int inputLen;          /* length of input block */
...
R_RSA_PUBLIC_KEY *publicKey;    /* RSA public key */
{
	...
	printf("\n in rsapublicfunc: inputlen = [%d] input as follow:\n",inputLen);
	for (i = 0; i < inputLen; i++)
		printf("%02x",input[i]);
	printf("\n\n");
...
R_RSA_PRIVATE_KEY *privateKey;  /* RSA private key */
{
	...
	printf("\n in rsaprivatefunc: inputlen = [%d] input as follow:\n",inputLen);
	for (i = 0; i < inputLen; i++)
		printf("%02x",input[i]);
	printf("\n\n");
