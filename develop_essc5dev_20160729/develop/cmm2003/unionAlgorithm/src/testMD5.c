...
#define	__BUILD_UNION_MD5_CORE__

#include <stdio.h>
#include <string.h>

#include "../include/UnionMD5.h"
#include "../include/UnionDataExch.h"
...
int UnionUncompressBitsIntoBCD(unsigned char *pBitsData,int LenOfBitsData,char *
pBCDData)
{
        int     i;
        unsigned char xxc;
...
int  UnionMD5(unsigned char *pData, unsigned long cbData, unsigned char *pDigest)
{
	...
	MD5Final(&Ctx, (unsigned char *)tmpDigest);
...
int main(int argc,char *argv[])
{
	...
	for (i = 0; i < argc; i++)
	{
		...
		UnionMD5(argv[i],strlen(argv[i]),tmpBuf);
		printf("[%s]\n[%s]\n",argv[i],tmpBuf);
	}
	*/
	UnionMD5File(argc[1],tmpBuf);
	printf("[%s]\n[%s]\n",argv[1],tmpBuf);
	return(0);
}		
