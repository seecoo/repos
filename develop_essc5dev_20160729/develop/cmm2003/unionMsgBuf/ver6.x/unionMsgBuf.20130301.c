...
typedef struct
{
	char				mdlNameOfMsgBuf[40+1];
	int				isMsgBufConnected;
	PUnionSharedMemoryModule	pgpdfzMsgBufMDL;
	PUnionMsgBufHDL			pgpdfzMsgBufHDL;
	unsigned char			*pgpdfzMsgBuf;
	
} TUnionMsgBufGrp;
typedef TUnionMsgBufGrp		*PUnionMsgBufGrp;

TUnionMsgBufGrp			gunionMsgBufGrp[MAX_NUM_MSGBUF_GROUP] = {
					{"UnionMsgBufMDL", 0, NULL, NULL, NULL}, {"", 0, NULL, NULL, NULL}, 
					{"", 0, NULL, NULL, NULL}, {"", 0, NULL, NULL, NULL}, {"", 0, NULL, NULL, NULL}, 
					{"", 0, NULL, NULL, NULL}, {"", 0, NULL, NULL, NULL}, {"", 0, NULL, NULL, NULL}};
char				gunionMDLNameOfMsgBuf[40+1] = "UnionMsgBufMDL";

int UnionSetMDLNameOfMsgBuf(char *mdlName)
{
	memset(gunionMDLNameOfMsgBuf, 0, sizeof(gunionMDLNameOfMsgBuf));
	strncpy(gunionMDLNameOfMsgBuf, mdlName, sizeof(gunionMDLNameOfMsgBuf) - 1);
	return(0);
}
int UnionReSetMDLNameOfMsgBuf()
{
	return(UnionSetMDLNameOfMsgBuf("UnionMsgBufMDL"));
}
...
int UnionSetMsgBufConnected(char *mdlName)
{
	int		i;

	for(i = 0; i < MAX_NUM_MSGBUF_GROUP; i++)
	{
		if(strcmp(gunionMsgBufGrp[i].mdlNameOfMsgBuf, mdlName) == 0)
		{
			gunionMsgBufGrp[i].pgpdfzMsgBufMDL = pgpdfzMsgBufMDL;
			gunionMsgBufGrp[i].pgpdfzMsgBufHDL = pgpdfzMsgBufHDL;
			gunionMsgBufGrp[i].pgpdfzMsgBuf = pgpdfzMsgBuf;
			gunionMsgBufGrp[i].isMsgBufConnected = 1;
			break;
		}
		if(strlen(gunionMsgBufGrp[i].mdlNameOfMsgBuf) == 0)
		{
			strcpy(gunionMsgBufGrp[i].mdlNameOfMsgBuf, mdlName);
			gunionMsgBufGrp[i].pgpdfzMsgBufMDL = pgpdfzMsgBufMDL;
			gunionMsgBufGrp[i].pgpdfzMsgBufHDL = pgpdfzMsgBufHDL;
			gunionMsgBufGrp[i].pgpdfzMsgBuf = pgpdfzMsgBuf;
			gunionMsgBufGrp[i].isMsgBufConnected = 1;
	...
	UnionAuditLog("in UnionSetMsgBufConnected:: mdlNameOfMsgBuf = [%s], gunionMsgBufGrp index = [%d]", UnionGetMDLNameOfMsgBuf(), i);

	return(0);
}

int UnionSwitchToSpecMsgBufConnected(char *mdlName)
{
	int		i;

	for(i = 0; i < MAX_NUM_MSGBUF_GROUP; i++)
	{
		if(strcmp(gunionMsgBufGrp[i].mdlNameOfMsgBuf, mdlName) == 0 && gunionMsgBufGrp[i].isMsgBufConnected == 1)
		{
			UnionAuditLog("in UnionSwitchToCurMsgBufConnected:: switch to [%s].", mdlName);
			pgpdfzMsgBufMDL = gunionMsgBufGrp[i].pgpdfzMsgBufMDL;
			pgpdfzMsgBufHDL = gunionMsgBufGrp[i].pgpdfzMsgBufHDL;
			pgpdfzMsgBuf = gunionMsgBufGrp[i].pgpdfzMsgBuf;
			gpdfzMsgBufMDLConnected	= gunionMsgBufGrp[i].isMsgBufConnected;
...
int UnionIsMsgBufConnected()
{
	int		i;

	for(i = 0; i < MAX_NUM_MSGBUF_GROUP; i++)
	{
		if(strcmp(gunionMsgBufGrp[i].mdlNameOfMsgBuf, UnionGetMDLNameOfMsgBuf()) == 0)
		{
			pgpdfzMsgBufMDL = gunionMsgBufGrp[i].pgpdfzMsgBufMDL;
			pgpdfzMsgBufHDL = gunionMsgBufGrp[i].pgpdfzMsgBufHDL;
			pgpdfzMsgBuf = gunionMsgBufGrp[i].pgpdfzMsgBuf;
...
int UnionReconnectMsgBufMDLAnyway()
{
	...
	if ((pgpdfzMsgBufMDL = UnionConnectSharedMemoryModule(UnionGetMDLNameOfMsgBuf(),
	...
	UnionSetMsgBufConnected(UnionGetMDLNameOfMsgBuf()); // add 2013-12-05
...
int UnionRemoveMsgBufMDL()
{
	...
	return(UnionRemoveSharedMemoryModule(UnionGetMDLNameOfMsgBuf()));
}	
...
int UnionBufferMessageWithNewHeaderForHsm(int applierMDLID, int seqReqTime, unsigned char * Msg,int LenOfMsg,long typeOfMsg,PUnionMessageHeader pnewHeader)
{
	...
	{
		UnionFreeMsgBufPos(indexOfMsg);  //add by hzh in 2012.04.10
		UnionUserErrLog("in UnionBufferMessageWithNewHeaderForHsm:: parameter error! lenOfMsg = [%d] pgpdfzMsgBufHDL->maxSizeOfMsg = [%d] typeOfMsg = [%ld]\n",LenOfMsg,pgpdfzMsgBufHDL->maxSizeOfMsg,typeOfMsg);
		return(errCodeParameter);
	}
...
// added for exchangePool 2013-12-09
// 向缓冲池数据队列放入一条数据
int UnionPutDataIntoExchangePool(int poolIndex, long dataType, int dataLen, char *data)
{
	int			index, ret;
	TUnionFreePosOfMsgBuf	PosOfMsgBuf;
	unsigned char 		*pAddr = NULL;
	PUnionMessageHeader	pheader = NULL;

	if ((ret = UnionConnectMsgBufMDL()) < 0)
	{
		UnionUserErrLog("in UnionPutDataIntoExchangePool:: UnionConnectMsgBufMDL!\n");
		return(ret);
	}

	PosOfMsgBuf.statusOfPos = dataType;
	index = poolIndex;

	pAddr = pgpdfzMsgBuf + index * (sizeof(TUnionMessageHeader) + pgpdfzMsgBufHDL->maxSizeOfMsg);
	pheader = (PUnionMessageHeader)pAddr;
	pheader->len = dataLen;
	time(&(pheader->time));
	pheader->provider = getpid();
	pheader->dealer = 0;
	pheader->msgIndex = index;
	pheader->type = dataType;

	memcpy(pAddr + sizeof(TUnionMessageHeader), data, dataLen);

	memcpy(PosOfMsgBuf.indexOfPos, &index, sizeof(int));
	if (msgsnd(pgpdfzMsgBufHDL->queueIDOfFreePos,  &PosOfMsgBuf,  sizeof(PosOfMsgBuf.indexOfPos), 0) < 0)
	{
		UnionSystemErrLog("in UnionPutDataIntoExchangePool:: msgsnd!\n");
		return(errCodeUseOSErrCode);
	}
...
// 向缓冲池数据队列取出一条数据
int UnionGetDataFromExchangePool(long dataType, int bufSize, char *data)
{
	int			index, ret;
	TUnionFreePosOfMsgBuf	PosOfMsgBuf;
	unsigned char 		*pAddr = NULL;
	...
	if ((ret = UnionConnectMsgBufMDL()) < 0)
	{
		UnionUserErrLog("in UnionGetDataIntoExchangePool:: UnionConnectMsgBufMDL!\n");
		return(ret);
	}

	memset(PosOfMsgBuf.indexOfPos, 0, sizeof(PosOfMsgBuf.indexOfPos));
	//for(i = 1; i < 10; i++) {usleep(1000);}
	if ((ret = msgrcv(pgpdfzMsgBufHDL->queueIDOfFreePos, &PosOfMsgBuf, sizeof(PosOfMsgBuf.indexOfPos), dataType, IPC_NOWAIT)) < 0)
	{
		UnionSystemErrLog("in UnionGetDataFromExchangePool:: msgrcv!\n");
		return(errCodeUseOSErrCode);
	}
	...
	pAddr = pgpdfzMsgBuf + index * (sizeof(TUnionMessageHeader) + pgpdfzMsgBufHDL->maxSizeOfMsg);
	pheader = (PUnionMessageHeader)pAddr;
	dataLen = (pheader->len > bufSize) ? bufSize : pheader->len;
	memcpy(data, pAddr + sizeof(TUnionMessageHeader), dataLen);

	return(dataLen);
}

// 向缓冲池已使用标识队列放入一条数据
int UnionPutFreeIndexToExchangePool(long dataType, int poolIndex)
{
	int				index, ret;
	TUnionOccupiedPosOfMsgBuf	occupiedPos;
	...
	if ((ret = UnionConnectMsgBufMDL()) < 0)
	{
		UnionUserErrLog("in UnionPutFreeIndexToExchangePool:: UnionConnectMsgBufMDL!\n");
		return(ret);
	}

	occupiedPos.typeOfMsg = dataType;
	index = poolIndex;
	memcpy(occupiedPos.indexOfPos, &index, sizeof(int));

	if(msgsnd(pgpdfzMsgBufHDL->queueIDOfOccupiedPos, &occupiedPos, sizeof(occupiedPos.indexOfPos), 0) < 0)
	{
		UnionSystemErrLog("in UnionPutFreeIndexToExchangePool:: msgsnd!\n");
		return(errCodeUseOSErrCode);
	}
...
// 向缓冲池已使用标识队列取出一条数据
int UnionGetFreeIndexFromExchangePool(long dataType)
{
	int				index, ret;
	TUnionOccupiedPosOfMsgBuf	occupiedPos;
	...
	if ((ret = UnionConnectMsgBufMDL()) < 0)
	{
		UnionUserErrLog("in UnionGetFreeIndexFromExchangePool:: UnionConnectMsgBufMDL!\n");
		return(ret);
	}
	...
	memset(occupiedPos.indexOfPos, 0, sizeof(occupiedPos.indexOfPos));
	if ((ret = msgrcv(pgpdfzMsgBufHDL->queueIDOfOccupiedPos, &occupiedPos, sizeof(occupiedPos.indexOfPos), dataType, 0)) < 0)
	{
		UnionSystemErrLog("in UnionGetFreeIndexFromExchangePool:: msgrcv!\n");
		return(errCodeUseOSErrCode);
	}
...
// 补充缓冲池记录
int UnionSupplyDataToExchangePool(long dataType, int dataLen, char *data)
{
	int			index;

	index = UnionGetFreeIndexFromExchangePool(dataType);
	if(index < 0)
	{
		return(index);
	}
	...
	{
		return(UnionPutDataIntoExchangePool(index, dataType, dataLen, data));
	}
}

// 读取缓冲池记录
int UnionReadDataFromExchangePool(long dataType, int bufSize, char *data)
{
	int			index = -1;

	index = UnionGetDataFromExchangePool(dataType, bufSize, data);
	if(index < 0)
	{
		return(index);
	}
	UnionPutFreeIndexToExchangePool(dataType, index);
	return(index);
