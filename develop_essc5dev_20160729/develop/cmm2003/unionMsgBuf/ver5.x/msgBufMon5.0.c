...
extern unsigned char		*pgunionMsgBuf;
extern PUnionMsgBufHDL		pgunionMsgBufHDL;
...
int RefreshExistMsgWin(int interval)
{
	...
	if ((pgunionMsgBuf == NULL) || (pgunionMsgBufHDL == NULL))
		return(-1);
	...
	for (posNum = 0,i = 0;;)
	{
		...
		waddstr(pgContentWin,Buf);
		++i;
		for (;posNum < pgunionMsgBufHDL->msgBufDef.maxNumOfMsg; posNum++)
		{
			if ((pmsgHeader = (PUnionMessageHeader)(pgunionMsgBuf+conMsgSizeInMsgBuf*posNum)) == NULL)
				return(-1);
		...
		if (posNum < pgunionMsgBufHDL->msgBufDef.maxNumOfMsg)
		{
			//if (interval < 10)
			//	sleep(10);
			//else
...
int RefreshFreePosWin(int interval)
{
	...
	for (posNum = 0,i = 0;;)
	{
		...
		++i;
		memset(Buf,0,sizeof(Buf));
		for (len = 0;posNum < pgunionMsgBufHDL->msgBufDef.maxNumOfMsg; posNum++)
		{
			if ((pmsgHeader = (PUnionMessageHeader)(pgunionMsgBuf+conMsgSizeInMsgBuf*posNum)) == NULL)
				return(-1);
			if ((pmsgHeader->type > 0) || (pmsgHeader->locked))
