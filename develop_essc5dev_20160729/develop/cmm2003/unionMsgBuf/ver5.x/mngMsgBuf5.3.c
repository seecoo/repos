...
int UnionHelp()
{
	...
	printf("  -resetallpos	read all pos available\n");
	return(0);
}
...
int main(int argc,char *argv[])
{
	...
	if (strcasecmp(argv[1],"-RESETALLPOS") == 0)
		return(ResetAllPos());
...
int ResetAllPos()
{
	...
	if (!UnionConfirm("Are you sure of reset all msgBuf pos?"))
		return(UnionTaskActionBeforeExit());
		
	if ((ret = UnionResetAllMsgBufPosAvailable()) < 0)
		printf("UnionResetAllMsgBufPosAvailable failure! ret = [%d]\n",ret);
	else
		printf("UnionResetAllMsgBufPosAvailable OK!\n");
	return(UnionTaskActionBeforeExit());
}
