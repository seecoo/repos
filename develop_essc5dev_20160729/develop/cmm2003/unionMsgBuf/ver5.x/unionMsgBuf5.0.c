...
long UnionApplyNewMsgIndexOfMsgBuf()
{
	...
	if ((pgunionMsgBufHDL->currentMsgIndex >= pgunionMsgBufHDL->msgBufDef.maxMsgIndex - 100000) || 
			(pgunionMsgBufHDL->currentMsgIndex <= 0))
		pgunionMsgBufHDL->currentMsgIndex = pgunionMsgBufHDL->msgBufDef.minMsgIndex;
	
	return(msgIndex = pgunionMsgBufHDL->currentMsgIndex = pgunionMsgBufHDL->currentMsgIndex + 100000);
#else
	if ((gunionMsgBufIndex >= 999) || (gunionMsgBufIndex <= 0))
		return(gunionMsgBufIndex = 1);
	else
		return(++gunionMsgBufIndex);
#endif
}
...
long UnionWriteNewMessage(unsigned char * msg,int lenOfMsg,long typeOfMsg,PUnionMessageHeader precvMsgHeader)
{
	...
	if ((pgunionMsgBufHDL == NULL) || (msg == NULL) || (lenOfMsg > pgunionMsgBufHDL->msgBufDef.maxSizeOfMsg) || (typeOfMsg <= 0))
	{
...
int UnionWriteOriginMessage(unsigned char *msg,int lenOfMsg,PUnionMessageHeader pOriMsgHeader)
{
	...
	{
		UnionUserErrLog("in UnionWriteOriginMessage:: wrong parameter!\n");
		return(errCodeParameter);
	}
