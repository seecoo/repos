...
long UnionFormKDBSvrServiceUniqueKey(PUnionKDBSvrServiceHeader pheader)
{
	...
	return(pheader->provider * 100 + pheader->index % 100);
}
...
int UnionApplyKDBService(TUnionKeyDBOperation operation,char keyDBType,unsigned char *keyRec,int sizeOfKeyRec)
{
	...
	UnionLog("in UnionApplyKeySynchronizeServiceNowait ::  pgunionKDBSvrBuf [%ld] conSizeOfEachKDBSvrRec[%ld] index [%d]\n",pgunionKDBSvrBuf,conSizeOfEachKDBSvrRec,index);
	printf("in UnionApplyKeySynchronizeServiceNowait ::  pgunionKDBSvrBuf [%ld] conSizeOfEachKDBSvrRec[%ld] index [%d]\n",pgunionKDBSvrBuf,conSizeOfEachKDBSvrRec,index);
...
int UnionSpierKeyDBSynchronizerService(char *ipAddr,int port,char *fullName,int operation,int resCode)
{
	return 0;
}
