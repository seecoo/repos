...
int UnionHelp()
{
	...
	printf(" Command\n");
	printf("	create spaceName hsmGrpID keyPosNum 	Create KeyPosSpace\n");
	printf("	drop spaceName hsmGrpID ...		Drop KeyPosSpace\n");
	
	printf("	read spaceName hsmGrpID keyPos ...	Read status of keyPos\n");
	printf("	available spaceName hsmGrpID		Read all available keyPos\n");
	printf("	occupied spaceName hsmGrpID		Read all occupied keyPos\n");
	printf("	allinvalid spaceName hsmGrpID		Print all invalid keyPos\n");
	printf("	puser spaceName hsmGrpID userName	Print all occupieed keyPos by user\n");
	
	printf("	selectavailable spaceName hsmGrpID	Read the minimum available keyPos\n");
	printf("	isavailable     spaceName hsmGrpID	decide whether a keyPos is available\n");
	
	printf("	occupy spaceName hsmGrpID keyPos userName ...	Set keyPos occupied\n");
	printf("	free spaceName hsmGrpID keyPos ...	Set keyPos available\n");
	printf("	freeUser spaceName hsmGrpID userName	Free all keyPos occupied by User\n");
	printf("	invalid spaceName hsmGrpID startKeyPos [endKeyPos] Set keyPos invalid\n");
	printf("	initickeyspace hsmGrpID versionNum groupNum indexNum\n");
	
	printf("	applyalllmkkeyspace hsmGrpID		Set LMK KeySpace for hsmGrp\n");
	
	printf("	status spaceName hsmGrpID\n");
	return(0);
}

int main(int argc,char **argv)
{
	int	hsmGrpIDPos = 3,spaceNamePos = 2,paramPos = 4;
	...
	//printf("argc = [%d] spaceNamePos = [%d] hsmGrpIDPos = [%d] paramPos = [%d]\n",argc,spaceNamePos,hsmGrpIDPos,paramPos);
	if (spaceNamePos >= argc)
	{
loopInputKeySpaceName:
	...
	{
		if (spaceNamePos != 1)
			strcpy(gname,argv[spaceNamePos]);
	}
	if (hsmGrpIDPos >= argc)
	{
loopInputHsmGrpID:
		...
		if (UnionIsQuit(ghsmGrp))
			return(UnionTaskActionBeforeExit());
		if ((atoi(ghsmGrp) < 0) || (atoi(ghsmGrp) > 999))
		{
			printf("Invalid hsmGrpID! reinput!\n");
			goto loopInputHsmGrpID;
		}
	}
	else
		strcpy(ghsmGrp,argv[hsmGrpIDPos]);
	UnionToUpperCase(gname);	
	...
	if (strcasecmp(argv[1],"READ") == 0)
		return(ReadKeyPos(argc-paramPos,&argv[paramPos]));
	...
	if (strcasecmp(argv[1],"ISAVAILABLE") == 0)
		return(IsAvailableKeyPos(argc-paramPos,&argv[paramPos]));
		
	if (strcasecmp(argv[1],"OCCUPY") == 0)
		return(OccupyKeyPos(argc-paramPos,&argv[paramPos]));
		
	if (strcasecmp(argv[1],"FREE") == 0)
		return(FreeKeyPos(argc-paramPos,&argv[paramPos]));
		
	if (strcasecmp(argv[1],"INVALID") == 0)
		return(InvalidKeyPos(argc-paramPos,&argv[paramPos]));
	
	if (strcasecmp(argv[1],"PUSER") == 0)
		return(PrintOccupiedByUser(argc-paramPos,&argv[paramPos]));
	
	if (strcasecmp(argv[1],"FREEUSER") == 0)
		return(FreeOccupiedByUser(argc-paramPos,&argv[paramPos]));
	...
	if (strcasecmp(argv[1],"INITICKEYSPACE") == 0)
		return(InitICKeySpace(argc-paramPos,&argv[paramPos]));
	UnionHelp();
...
int CreateKeyPosSpace(int argc,char *argv[])
{
	...
	if (argc > 1)
	{
		strcpy(ghsmGrp,argv[1]);
	}
	else
	{
		if (UnionIsQuit(p = UnionInput("Input HsmGrpID(Exit to quit)::")))
			return(UnionTaskActionBeforeExit());
		strcpy(ghsmGrp,p);
	}
	if (argc > 2)
...
createKeyPosSpace:
	if (UnionExistSJL06KeySpaceFile(ghsmGrp,gname))
	{	
		if (!UnionConfirm("A keyPosSpace named [%s.%s] has existed, are you sure of rewriting the old keyPosSpace?"
			,ghsmGrp,gname))
		{
			if (argc > 0)
				return(UnionTaskActionBeforeExit());
