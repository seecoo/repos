...
int UnionLongConnSJL06Cmd(int sckConn,PUnionSJL06 pSJL06,char *reqStr,int lenOfReqStr,char *resStr,int sizeOfResStr)
{
	...
	else
	{
		...
		memcpy(errCode,tmpBuf + pSJL06->staticAttr.lenOfLenFld + pSJL06->staticAttr.lenOfMsgHeader + 2,2);
		if (strncmp(errCode,"00",2) != 0)
		{
			UnionUserErrLog("in UnionLongConnSJL06Cmd:: Hsm [%s] ReqLen [%d]\nCmdReq=[%s]\n",
				pSJL06->staticAttr.ipAddr,lenOfReqStr,reqStr);
		}
		else
		{		
			UnionDebugLog("in UnionLongConnSJL06Cmd:: Hsm [%s] ReqLen [%d]\nCmdReq=[%s]\n",
				pSJL06->staticAttr.ipAddr,lenOfReqStr,reqStr);
		}
		...
		if (strncmp(errCode,"00",2) != 0)
		{
			UnionUserErrLog("in UnionLongConnSJL06Cmd:: Hsm [%s] ResLen [%d]\nCmdRes=[%s]\n",
				pSJL06->staticAttr.ipAddr,hsmStrLen,resStr);
		}
		else	
		{
			UnionDebugLog("in UnionLongConnSJL06Cmd:: Hsm [%s] ResLen [%d]\nCmdRes=[%s]\n",
				pSJL06->staticAttr.ipAddr,hsmStrLen,resStr);
		}
	}
	alarm(0);
...
abnormalExit:
	alarm(0);
	UnionUserErrLog("in UnionLongConnSJL06Cmd:: Hsm [%s] CmdStrLen [%d]\nCmdReq=[%s]\n",
			pSJL06->staticAttr.ipAddr,lenOfReqStr,reqStr);
...
abnormalExit:
	alarm(0);
	UnionUserErrLog("in UnionLongConn2HexLenSJL06Cmd:: Hsm [%s] CmdStrLen [%d]\nCmdReq=[%s]\n",
			pSJL06->staticAttr.ipAddr,lenOfReqStr,reqStr);
	pSJL06->dynamicAttr.status = conAbnormalSJL06;
	return(errSJL06Abnormal);
}
...
int UnionLongConnSJL06CmdAnyway(int sckConn,PUnionSJL06 pSJL06,char *reqStr,int lenOfReqStr,char *resStr,int sizeOfResStr)
{
	...
	else
	{
		...
		if (strncmp(errCode,"00",2) != 0)
		{
			UnionUserErrLog("in UnionLongConnSJL06CmdAnyway:: Hsm [%s] ReqLen [%d]\nCmdReq=[%s]\n",
				pSJL06->staticAttr.ipAddr,lenOfReqStr,reqStr);
		}
		else
		{		
			UnionDebugLog("in UnionLongConnSJL06CmdAnyway:: Hsm [%s] ReqLen [%d]\nCmdReq=[%s]\n",
				pSJL06->staticAttr.ipAddr,lenOfReqStr,reqStr);
		}
		memcpy(resStr,tmpBuf + pSJL06->staticAttr.lenOfLenFld + pSJL06->staticAttr.lenOfMsgHeader,hsmStrLen);
		resStr[hsmStrLen] = 0;
		if (strncmp(errCode,"00",2) != 0)
		{
			UnionUserErrLog("in UnionLongConnSJL06CmdAnyway:: Hsm [%s] ResLen [%d]\nCmdRes=[%s]\n",
				pSJL06->staticAttr.ipAddr,hsmStrLen,resStr);
		}
		else	
		{
			UnionDebugLog("in UnionLongConnSJL06CmdAnyway:: Hsm [%s] ResLen [%d]\nCmdRes=[%s]\n",
				pSJL06->staticAttr.ipAddr,hsmStrLen,resStr);
		}
	}
	alarm(0);
...
abnormalExit:
	alarm(0);
	UnionUserErrLog("in UnionLongConnSJL06CmdAnyway:: Hsm [%s] CmdStrLen [%d]\nCmdReq=[%s]\n",
			pSJL06->staticAttr.ipAddr,lenOfReqStr,reqStr);
	pSJL06->dynamicAttr.status = conAbnormalSJL06;
	return(errSJL06Abnormal);
}
