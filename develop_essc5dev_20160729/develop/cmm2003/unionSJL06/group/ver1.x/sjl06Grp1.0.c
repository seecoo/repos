...
int UnionCheckStatusOfWorkingSJL06()
{
	...
	for (index = 0; index < conMaxNumOfSJL06; index++)
	{
		...
		if (strcmp(pSJL06->staticAttr.hsmCmdVersion,conHsmCmdVersionSJL06) == 0)
			ret = SJL06Cmd01(-1,pSJL06,errCode);
		else if (strcmp(pSJL06->staticAttr.hsmCmdVersion,conHsmCmdVersionRacal) == 0)
			ret = RacalCmdNC(-1,pSJL06,hsmVersion,errCode);
		else
			continue;
		if ((ret < 0) || (strncmp(errCode,"00",2) != 0))
