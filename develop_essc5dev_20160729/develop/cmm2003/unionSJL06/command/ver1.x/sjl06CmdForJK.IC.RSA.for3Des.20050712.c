...
int SJL06Cmd3B(PUnionSJL06Server psjl06Server,char *pk,char *keyByMK,char *keyByPK,int sizeOfBuf,char *checkValue,char *errCode)
{
	...
	{
		UnionUserErrLog("in SJL06Cmd3B:: UnionSJL06ServerService!\n");
		return(ret);
	}
	hsmCmdBuf[ret] = 0;
...
int SJL06Cmd3A(PUnionSJL06Server psjl06Server,int vkIndex,int lenOfKeyByPK,char *keyByPK,char *keyByMK,char *checkValue,char *errCode)
{
	...
	if ((ret = UnionSJL06ServerService(psjl06Server,hsmCmdBuf,hsmCmdLen,hsmCmdBuf,sizeof(hsmCmdBuf))) < 0)
	{
		UnionUserErrLog("in SJL06Cmd3A:: UnionSJL06ServerService!\n");
		return(ret);
	}
	hsmCmdBuf[ret] = 0;
...
int SJL06Cmd11(PUnionSJL06Server psjl06Server,TUnionDesKeyLength keyLen,char *bmkIndex,char *bmkValue,char *errCode)
{
	...
	switch (keyLen)
	{
		...
		case	con192BitsDesKey:
			...
			break;
		default:
			UnionUserErrLog("in SJL06Cmd1B:: invalid key length [%d]\n",keyLen);
			return(errCodeParameter);
	}
	memcpy(hsmCmdBuf+hsmCmdLen,bmkIndex,3);
	hsmCmdLen += 3;
	...
	switch (keyLen)
	{
		case	con128BitsDesKey:
			...
			return(32);
		case	con192BitsDesKey:
			memcpy(bmkValue,hsmCmdBuf+4,48);
			return(48);
		default:
			UnionUserErrLog("in SJL06Cmd1B:: invalid key length [%d]\n",keyLen);
			return(errCodeParameter);
...
int SJL06Cmd1A(PUnionSJL06Server psjl06Server,TUnionDesKeyLength keyLen,char *bmkIndex,char *keyValue,char *errCode)
{
	...
	switch (keyLen)
	{
		...
		case	con128BitsDesKey:
			...
			return(32);
		case	con192BitsDesKey:
			memcpy(keyValue,hsmCmdBuf+4,48);
			return(48);
		default:
			UnionUserErrLog("in SJL06Cmd1B:: invalid key length [%d]\n",keyLen);
			return(errCodeParameter);
...
int SJL06Cmd16(PUnionSJL06Server psjl06Server,TUnionDesKeyLength keyLen,char *bmkIndex,char *keyValue,char *checkValue,char *errCode)
{
	...
	switch (keyLen)
	{
		...
		case	con192BitsDesKey:
			memcpy(keyValue,hsmCmdBuf+4,48);
			memcpy(checkValue,hsmCmdBuf+4+48,16);
			return(48);
		default:
			UnionUserErrLog("in SJL06Cmd16:: invalid key length [%d]\n",keyLen);
			return(errCodeSJL06MDL_InvalidHsmRetLen);
...
int SJL06Cmd17(PUnionSJL06Server psjl06Server,TUnionDesKeyLength keyLen,char *bmkIndex1,char *bmkIndex2,char *keyValue1,char *keyValue2,char *errCode)
{
	int		ret;
	char		hsmCmdBuf[512+1];
	int		hsmCmdLen = 0;
...
int SJL06Cmd61(PUnionSJL06Server psjl06Server,TUnionDesKeyLength keyLen1,TUnionDesKeyLength keyLen2,char *bmkIndex1,char *bmkIndex2,char *keyValue1,char *keyValue2,char *pin1,char *pan,char *pin2,char *errCode)
{
	int		ret;
	char		hsmCmdBuf[512+1];
	int		hsmCmdLen = 0;
...
int SJL06Cmd1B(PUnionSJL06Server psjl06Server,TUnionDesKeyLength keyLen,char *bmkIndex,char *keyValue,char *errCode)
{
	...
	switch (keyLen)
	{
		case	con64BitsDesKey:
			...
			return(32);
		case	con128BitsDesKey:
			memcpy(keyValue,hsmCmdBuf+4,64);
			return(64);
		case	con192BitsDesKey:
			memcpy(keyValue,hsmCmdBuf+4,96);
			return(96);
		default:
			UnionUserErrLog("in SJL06Cmd1B:: invalid key length [%d]\n",keyLen);
			return(errCodeParameter);
...
int SJL06Cmd81(PUnionSJL06Server psjl06Server,TUnionDesKeyLength keyLen,char *bmkIndex,char *mak,char *mac,int macDataLength,char *macData,char *errCode)
{
	...
	if ((ret = SJL06Cmd80(psjl06Server,keyLen,bmkIndex,mak,macDataLength,macData,localMac,errCode)) < 0)
	{
