...
int ConnectHsmSJL05(char *ipAddr,int port)
{
	...
	{
		UnionUserErrLog("in ConnectHsmSJL05:: UnionCreateSocketClient [%s][%d]!\n",ipAddr,port);
	}
...
int ExcuteHsmSJL05Cmd(char *reqStr,int lenOfReqStr,char *resStr,int sizeOfResStr)
{
	if (gsjl06HsmSckHDL >= 0)
		return(UnionCommWithSpecSckHDLInBitsFormatWithoutLen(gsjl06HsmSckHDL,(unsigned char *)reqStr,lenOfReqStr,
				(unsigned char *)resStr,sizeOfResStr));
	else
		return(UnionCommWithSpecSvrInBitsFormatWithoutLen(UnionReadStringTypeRECVar("ipAddrOfHsmModel"),
				UnionReadIntTypeRECVar("portOfHsmModel"),
				(unsigned char *)reqStr,lenOfReqStr,(unsigned char *)resStr,sizeOfResStr));
}
