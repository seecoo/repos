...
int UnionDealServiceCode0H08(PUnionHsmGroupRec phsmGroupRec)
{
	...
	{
		UnionUserErrLog("in UnionDealServiceCode0H08:: UnionReadRequestXMLPackageValue[%s]!\n","body/modeID");
		return(ret);
	}
	...
	if ((ret = UnionReadRequestXMLPackageValue("body/desProtectKey",desProtectKey,sizeof(desProtectKey))) > 0)
	{
		...
		if (ret != 6 && ret != 16)
		{
			UnionUserErrLog("in UnionDealServiceCode0H08:: desProtectCV[%s] length error, must is 6 or 16!\n",desProtectCV);
	...
	if ((ret = UnionReadRequestXMLPackageValue("body/sm4ProtectKey",sm4ProtectKey,sizeof(sm4ProtectKey))) > 0)
	{
		...
		if (ret != 6 && ret != 16)
		{
			UnionUserErrLog("in UnionDealServiceCode0H08:: sm4ProtectCV[%s] length error, must is 6 or 16!\n",sm4ProtectCV);
	...
	if (strlen(desProtectKey) >= 16 || strlen(sm4ProtectKey) >= 16)
	{
		...
		{
			UnionUserErrLog("in UnionDealServiceCode0H08:: UnionSelectRealDBRecord[%s]!\n",sql);
			return(ret);
		}
		...
		{
			UnionUserErrLog("in UnionDealServiceCode0H08:: UnionReadXMLPackageValue[%s]!\n","totalNum");
			return(ret);
		}
		else
		...
		for (i = 1; i <= totalNum; i++)
		{
			...
			{
				UnionUserErrLog("in UnionDealServiceCode0H08:: UnionLocateXMLPackage[%s]!\n","totalNum");
				return(ret);
			}
			...
			{
				UnionUserErrLog("in UnionDealServiceCode0H08:: UnionReadXMLPackageValue[%s][%d]!\n","modeID",i);
				return(ret);
			}
			...
			if (strlen(tmpBuf) > 0)
			{
				...
				{
					UnionUserErrLog("in UnionDealServiceCode0H08:: modeID[%s] desProtectCV[%s] != modeID[%s] desProtectCV[%s] [%d]!\n",tmpModeID,tmpBuf,modeID,desProtectCV,i);
			...
			if (strlen(tmpBuf) > 0)
			{
				...
				{
					UnionUserErrLog("in UnionDealServiceCode0H08:: modeID[%s] sm4ProtectCV[%s] != modeID[%s] sm4ProtectCV[%s] [%d]!\n",tmpModeID,tmpBuf,modeID,sm4ProtectCV,i);
	...
	memset(sql,0,sizeof(sql));
	sprintf(sql,"update lmkProtectMode set modeName = '%s',desProtectKey = '%s',desProtectCV = '%s',sm4ProtectKey = '%s',sm4ProtectCV = '%s',remark = '%s' where modeID = '%s'",modeName,desProtectKey,desProtectCV,sm4ProtectKey,sm4ProtectCV,remark,modeID);
	if ((ret = UnionExecRealDBSql(sql)) < 0)
	{
		UnionUserErrLog("in UnionDealServiceCode0H08:: UnionExecRealDBSql[%s]!\n",sql);
		UnionSetResponseRemark("修改LMK保护方式失败");
		return(ret);
	}
